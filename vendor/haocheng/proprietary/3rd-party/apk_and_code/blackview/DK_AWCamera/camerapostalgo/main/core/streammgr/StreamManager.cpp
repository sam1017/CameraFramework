...
auto StreamManager::initialize() -> status_t {
    ...
    mPipeConfigs.add(StreamType::VIDEO, new PipelineConfigure());
#endif
    const char* tmpRstInstanceName = android::String8::format("%s-StreamMgr-RstHdl",
        mInstanceName.c_str()).string();
    std::string rstInstanceName(tmpRstInstanceName ? tmpRstInstanceName : "");
    mResultHandler = new ResultHandler(rstInstanceName);
    ...
    const char* tmpCbInstanceName = android::String8::format("%s-StreamMgr-CbHdl",
        mInstanceName.c_str()).string();
    std::string cbInstanceName(tmpCbInstanceName ? tmpCbInstanceName : "");
    mCallbackHandler = new CallbackHandler(cbInstanceName);
...
auto StreamManager::getCommonMetadata(
    ...
    switch (type) {
        case StreamType::CAPTURE:
            return &mCommonCaptureParams;
        case StreamType::PREVIEW:
            return &mCommonPreviewParams;
#ifdef FT_AI_CAMERA_FEATURE
        case StreamType::VIDEO:
            return &mCommonVideoParams;
#endif
        default:
            ALOGE("%s: wrong type: %d, return mCommonPreviewParams", __FUNCTION__, type);
            return &mCommonPreviewParams;
    }
}
...
Status StreamManager::configure(const FeatureConfig& config,
    ...
    if (sfp != nullptr) {
        IMetadata* data = nullptr;
        switch (type) {
        case StreamType::CAPTURE:
            data = &mCommonCaptureParams;
            break;
        case StreamType::PREVIEW:
            data = &mCommonPreviewParams;
            break;
#ifdef FT_AI_CAMERA_FEATURE
        case StreamType::VIDEO:
            data = &mCommonVideoParams;
            break;
#endif
        default:
            ALOGE("%s: wrong type: %d, set as mCommonPreviewParams", __FUNCTION__, type);
            data = &mCommonPreviewParams;
            break;
        }

        ParamConvert::getInstance()->convertFromParam(*sfp,*data);
...
//    }
    ...
    for (auto si : infos) {
        ...
        halFormat = android::mapPublicFormatToHalFormat(
                publicFormat);
        nativeDataspace = android::mapPublicFormatToHalDataspace(
                publicFormat);
...
auto StreamManager::configureStreams(const StreamType type,
    ...
    const IMetadata& metadataPtr = *getCommonMetadata(type);
...

    {
        ...
        for (auto surface : appSurfaces) {
            ...
            if (type == StreamType::VIDEO) {
                native_window_set_buffer_count(anw, 10);
                surface->setMaxDequeuedBufferCount(7);
                surface->setDequeueTimeout(100000000);
            }
#endif
            int32_t ap_format = ANativeWindow_getFormat(anw);
            if (type == StreamType::PREVIEW) {
                //set app preview surface buffer count is 3.
                err = native_window_set_buffer_count(anw, MAX_LOCK_BUFFER_COUNT);
                if (err != android::NO_ERROR) {
                    ALOGE(
                            "[%s] Unable to native_window_set_buffer_count: %s (%d)",
                            __func__, strerror(-err), err);
                }
                if (ap_format == HAL_PIXEL_FORMAT_RGB_565) {
                    //set app preview surface buffer format is RGBA8888.
                    ALOGD("set app preview surface buffer format is RGBA8888");
                    err = native_window_set_buffers_format(anw, HAL_PIXEL_FORMAT_RGBA_8888);
                    if (err != android::NO_ERROR) {
                        ALOGE(
                                "[%s] Unable to native_window_set_buffers_format: %s (%d)",
                                __func__, strerror(-err), err);
                        status_t ret_mode =
                                native_window_set_scaling_mode(anw,
                                                               NATIVE_WINDOW_SCALING_MODE_SCALE_TO_WINDOW);
                        if (ret_mode != android::OK) {
                            ALOGE("[%s] Unable to native_window_set_scaling_mode: %s (%d)",
                                  __func__, strerror(-err), err);
                        }
                    }
                }
