...
#define LOG_TAG "PostAlgo/GrallocHelper"
//
#include "MyUtils.h"
...
#define MY_QUERY(_extra_get_, _field_) \
    ...
    MY_QUERY(STRIDE         , stride);
    MY_QUERY(HEIGHT         , height);
        //
#undef  MY_QUERY
    //
...
queryStaticInfo(buffer_handle_t const bh, MyStaticInfo& info)
{
    ...
    case HAL_PIXEL_FORMAT_YCrCb_420_SP:
        {
            // Gralloc apply vertical stride on 64bytes alignment
            info.planes.resize(2);
            {// Y
                MyStaticInfo::Plane& plane = info.planes.editItemAt(0);
                plane.rowStrideInBytes = extra.stride;
                plane.sizeInBytes = extra.stride* extra.rowNum;
                //
                checkPlaneSize(info, 0, (extra.rowNum));
            }
            {// CrCb
                ...
                plane.rowStrideInBytes = extra.stride;
                plane.sizeInBytes = (extra.stride* extra.rowNum)>>1;
                //
    ...
    case HAL_PIXEL_FORMAT_NV12:
        {
            ...
            {// Y
                MyStaticInfo::Plane& plane = info.planes.editItemAt(0);
                plane.rowStrideInBytes = extra.stride;
                plane.sizeInBytes = extra.stride* extra.height;
                //
                checkPlaneSize(info, 0, (extra.rowNum));
            }
            {// CrCb
                MyStaticInfo::Plane& plane = info.planes.editItemAt(1);
                plane.rowStrideInBytes = extra.stride;
                plane.sizeInBytes = (extra.stride* extra.height)>>1;
                //
                checkPlaneSize(info, 1, (extra.rowNum>>1));
            }
        }break;
    ...
    case HAL_PIXEL_FORMAT_YCBCR_P010:
        {
            ...
            {// Y
                MyStaticInfo::Plane& plane = info.planes.editItemAt(0);
                plane.rowStrideInBytes = extra.rowStrideInPixels*2;
                plane.sizeInBytes = extra.stride*2 * extra.height;
                //
                checkPlaneSize(info, 0, (extra.rowNum));
            }
            {// CrCb
                MyStaticInfo::Plane& plane = info.planes.editItemAt(1);
                plane.rowStrideInBytes = extra.rowStrideInPixels*2;
                plane.sizeInBytes = ((extra.stride*2 * extra.height)>>1);
                //
                checkPlaneSize(info, 1, (extra.rowNum>>1));
            }
        }break;
    ...
    case HAL_PIXEL_FORMAT_YV12:
        {
            ...
            {// Cr
                MyStaticInfo::Plane& plane = info.planes.editItemAt(1);
                plane.rowStrideInBytes = extra.stride >> 1;
                plane.sizeInBytes = (extra.stride * extra.height) >> 2;
                //
                checkPlaneSize(info, 1, (extra.rowNum>>1));
            }
            {// Cb
                ...
                if ( (extra.producerUsage &  GRALLOC1_USAGE_SMVR) != 0)
                {
                    ...
                    plane.sizeInBytes = ((((((info.widthInPixels-1)>>5)+1)<<5) * ((((info.heightInPixels-1)>>5)+1)<<5) * 3/2) - ((extra.stride * extra.height) + ((extra.stride * extra.height)>>2)));
                }
            }
        }break;
    //
    ...
    case HAL_PIXEL_FORMAT_CAMERA_OPAQUE:
        {
            ...
            //
            {
                ...
                #define ALIGN_STRIDE(w, a) (((w + (a-1)) / a) * a)
                plane.rowStrideInBytes  = ALIGN_STRIDE(extra.rowStrideInPixels, 64)*10/8;
                #undef ALIGN_STRIDE
...
)
{
    ...
    const Rect outRect(width,height);

    status_t error = GraphicBufferMapper::get().lock(handle, usage, outRect,vaddr);
