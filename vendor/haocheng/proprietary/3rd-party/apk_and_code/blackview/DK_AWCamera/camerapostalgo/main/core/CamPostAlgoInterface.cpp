...
Status CamPostAlgoInterface::config(const vector<FeatureConfig>& configs,
...
#ifdef FT_AI_CAMERA_FEATURE
    std::vector<AlgoType> videoAlgos;
#endif
    StreamType type;
    ...
    const char* tmpInterfaceName = getInterfaceName().string();
    std::string interfaceName(tmpInterfaceName ? tmpInterfaceName: "");
...
#ifdef FT_AI_CAMERA_FEATURE
        case StreamType::VIDEO:
            videoAlgos.insert(videoAlgos.end(), algos.begin(), algos.end());
            break;
#endif
        default:
...
#ifdef FT_AI_CAMERA_FEATURE
    mVideoProcessor.init(NSPA::FeatureInitParam {
            .mType = StreamType::VIDEO, .mInterfaceId = mId, .mAlgos =
                    videoAlgos, .mStreamManager = mStreamManager });
    NSPA::FeatureConfigParam vParams;
    ret = mVideoProcessor.config(vParams);
    if (ret != MTRUE) {
        ALOGE("Video processor configure failed!");
        return Status::fromExceptionCode(Status::EX_ILLEGAL_STATE);
    }
#endif
...
#ifdef FT_AI_CAMERA_FEATURE
    Vector<sp<android::Surface>> videos;
#endif
...
#ifdef FT_AI_CAMERA_FEATURE
        case StreamType::VIDEO:
            videos.push_back(stream.getSurface());
            break;
#endif
        default:
...
#ifdef FT_AI_CAMERA_FEATURE
    res = mStreamManager->configureStreams(StreamType::VIDEO, videos);
    if (res != android::NO_ERROR) {
        ALOGE("%s: error to configure streams type %d, %s(%d)", __FUNCTION__,
                StreamType::VIDEO, strerror(-res), res);
        return Status::fromExceptionCode(res);
    }
#endif
    return Status::ok();
