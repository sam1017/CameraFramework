...
MBOOL FeatureNode::dumpData(RequestPtr &request, IImageBuffer *buffer, T fmt, Ts... args)
{
    TRACE_FUNC_ENTER();
    MBOOL ret = MFALSE;
    if( buffer && fmt )
...
MBOOL FeatureNode::dumpNamedData(RequestPtr &request, IImageBuffer *buffer, const char *name)
{
    ...
    if( buffer && name )
    {
        ...
        if(snprintf(path, sizeof(path), "/sdcard/dump/%04d_%s_%dx%d.bin", request->getRequestNo(), name, width, height)<0) {
           MY_LOGE("snprintf[/sdcard/dump/%04d_%s_%dx%d.bin] failed", request->getRequestNo(), name, width, height);
        }
