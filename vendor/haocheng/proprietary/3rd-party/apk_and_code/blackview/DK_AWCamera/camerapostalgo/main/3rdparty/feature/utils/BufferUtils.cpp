...
void BufferUtils::deallocBuffer(IImageBuffer* pBuf) {
    if (pBuf == NULL) {
...
void BufferUtils::dumpBuffer(IImageBuffer* buffer, char* fileNamePrefix) {
    ...
    int err = sprintf(fileName, "%s/%d_%s_f%d_w%d_h%d_s%d.raw", path, ++dumpCount, fileNamePrefix,
        buffer->getImgFormat(), size.w, size.h, size.w);
    if (err < 0) {
        ALOGE("fail to sprintf fileName");
        return;
    }
...
void BufferUtils::dumpBufferX(unsigned char* buffer, char* fileNamePrefix, int size) {
    ...
    char fileName[128];
    int err = sprintf(fileName, "%s/%d_%s_f%d_w%d_h%d_s%d.yv12", path, ++dumpCount, fileNamePrefix,
