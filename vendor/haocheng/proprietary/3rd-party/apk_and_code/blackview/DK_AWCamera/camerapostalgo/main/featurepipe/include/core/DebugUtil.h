...
static PRINTF_ARGS_FN TO_PRINTF_ARGS_FN(const char *fmt)
{
...
static PRINTF_ARGS_FN TO_PRINTF_ARGS_FN(const char *fmt, Args... args)
{
    return [=](char *buf, size_t n) -> int
          {
...
static int mySnprintf(char *buf, size_t n, const PRINTF_ARGS_FN &argsFn)
{
  return argsFn ? argsFn(buf, n) : -1;
}
