...
StringUtil::StringUtil(const StringUtil &src)
{
  TRACE_FUNC_ENTER();
  copy(src);
  TRACE_FUNC_EXIT();
...
int StringUtil::_printf(const PRINTF_ARGS_FN &argsFn)
{
  TRACE_FUNC_ENTER();
  int len = 0;
  if( mPtr && !mFull )
  {
    len = mySnprintf(mPtr+mLen, mSize-mLen, argsFn);
...
void StringUtil::copy(const StringUtil &src)
{
  TRACE_FUNC_ENTER();
  if( mSize != src.mSize )
  {
    ...
    if( mSize != src.mSize )
    {
      mPtr = new char[src.mSize];
      mSize = src.mSize;
      mPtr[0] = '\0';
    }
  }
  memcpy(mPtr, src.mPtr, src.mLen);
  mLen = src.mLen;
  mFull = src.mFull;
  TRACE_FUNC_EXIT();
}
