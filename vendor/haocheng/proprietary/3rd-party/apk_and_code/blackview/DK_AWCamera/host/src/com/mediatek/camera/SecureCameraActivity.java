...
public class SecureCameraActivity extends CameraActivity {
    ...
    protected void goToGallery(Uri uri) {
        if (uri == null) {
            LogHelper.i(TAG, "uri is null, can not go to gallery");

            //add by huangfei for exit securecamera slowly when watermark on start
            finish();
            //add by huangfei for exit securecamera slowly when watermark on end
