...
public class QcomSuperCaptureResult implements SuperCaptureResultInterface {

    private static final String TAG = "QcomSuperCaptureResult";
    ...
    public void init(Context context, Size photoSize, Size previewSize, Rect activeRect) {
        ...
        synchronized (this) {
            Log.d(TAG, "SuperResolutionJni.Init");
            if (!mInited) {
...
//                int ret_enhance = JniInterface.aw_enhance_initialize(mPhotoSize.getWidth(), mPhotoSize.getHeight());
//                Log.d(TAG, "JniInterface.init ret_enhance = "+ret_enhance);
                if (ret == 0) {
                    mInited = true;
                } else {
                    mInited = false;
...
//                JniInterface.aw_enhance_uninit();
                mInited = false;
            }
        }
    }
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    if (mPhotoForamt == ImageFormat.JPEG) {
                        ...
                        captureYuv = ImageFormatUtil.getDataFromImage(imagePlanes, crop, format, ImageFormatUtil.COLOR_FormatNV21);
    ...
    private synchronized void procImageEffect(byte[] jpeg, byte[] data, int num) {
        ...
        String fileName = SuperUtil.DUMP_FILE_PATH + SuperUtil.mDumpFileTitle + "_" + num + ".jpg";
...
//                JniInterface.aw_enhance_process(result, result_enhance, mPhotoSize.getWidth(), mPhotoSize.getHeight(), 1.4f, 1.1f);
                Log.i(TAG, "aw_enhance_process end");
            }
