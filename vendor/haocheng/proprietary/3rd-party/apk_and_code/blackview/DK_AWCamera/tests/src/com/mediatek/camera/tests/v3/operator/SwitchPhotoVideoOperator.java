...
public class SwitchPhotoVideoOperator extends Operator {
    ...
    private void switchToMode(int index) {
        ...
        switch (index) {
            case INDEX_VIDEO:
                ...
                UiObject2 shutterVideo = Utils.findObject(Utils.getShutterSelector().desc
                        ("Video"));
                if (shutterVideo == null) {
                    clickButton("Video");
                }
                break;
            case INDEX_PHOTO:
                Utils.getUiDevice().swipe(startX, startY, endX, endY, swipeSteps);
                UiObject2 shutterPhoto = Utils.findObject(Utils.getShutterSelector().desc
                        ("Picture"));
    ...
    private void clickButton(String s) {
        UiObject2 mClickButton = Utils.getUiDevice().findObject(By.text(s));
