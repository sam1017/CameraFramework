...
public class ImageUtil {
    ...
    public static byte[] getJepgFromImage(Image image, int rotation) {
        if (image == null) {
            return null;
        }
        ByteBuffer yBuffer = image.getPlanes()[0].getBuffer();
        ByteBuffer uBuffer = image.getPlanes()[1].getBuffer();
        ByteBuffer vBuffer = image.getPlanes()[2].getBuffer();
        final byte[] yBytes, uBytes, vBytes;
        yBytes = new byte[yBuffer.remaining()];
        vBytes = new byte[vBuffer.remaining()];
        uBytes = new byte[uBuffer.remaining()];
        yBuffer.get(yBytes);
        ...
        int rowStride0 = image.getPlanes()[0].getRowStride();
        int rowStride1 = image.getPlanes()[1].getRowStride();

        return YuvEncodeJni.getInstance().EncodeYuvToJpeg(
                yBytes,uBytes,vBytes, format, image.getWidth(), image.getHeight(),
                rowStride0, rowStride1, rotation);
    }
    ...
    public static Bitmap getBitmapFromImage(Image image, int jpgRotate, boolean mirror) {
        if (image == null) {
            return null;
        }
        ByteBuffer yBuffer = image.getPlanes()[0].getBuffer();
        ...
        if (mirror) {
            bmp = mirrorBitmap(bmp);
        }

        return bmp;
    }

    public static Bitmap createSourceBitmap(byte[] jpegData, int inSampleSize, boolean mirror) {
	android.util.Log.d("geek","createSourceBitmap mirror ="+mirror);
        ...
        if (bitmap != null && !bitmap.isRecycled()) {
            int degree = getJpegDegree(jpegData);
    ...
    private static Bitmap mirrorBitmap(Bitmap bitmap) {
        ...
        matrix.setScale(-1, 1);
        Bitmap mirror = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix, true);
        bitmap.recycle();
        return mirror;
    }

    public static int getJpegDegree(byte[] jpegData) {
        int degree = 0;
        try {
