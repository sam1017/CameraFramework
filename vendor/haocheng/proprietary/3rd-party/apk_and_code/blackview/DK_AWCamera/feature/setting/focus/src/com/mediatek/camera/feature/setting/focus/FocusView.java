...
//add by huangfei for manual focus show time start
import com.mediatek.camera.CameraActivity;
//add by huangfei for manual focus show time end
...
public class FocusView extends RotateLayout implements IFocusView {
    ...
    private static final int SCALING_DOWN_STAY_TIME = 2000;	//bv wuyonglin modify for bug4492 20210329
    ...
    //*/ hct.huangfei, 20201111. manual focus show time.
    private boolean highlightFocusView = false;
    //*/
    ...
    @Override
    public void startActiveFocus(boolean locked) {
        if (getFocusState() != FocusViewState.STATE_IDLE || getHandler() == null) {
        ...
        mFocusRing.setVisibility(VISIBLE);
        if (!locked) {
            mFocusRing.setImageDrawable(this.getResources().getDrawable(R.drawable.bv_ic_touch_focus));
        } else {
            mFocusRing.setImageDrawable(this.getResources().getDrawable(R.drawable.bv_ic_lock_focus));
        }
        setVisibility(VISIBLE);
        animate().withLayer().setDuration(0).scaleX(1.2f).scaleY(1.2f).alpha(1.0f);
        setFocusState(FocusViewState.STATE_ACTIVE_FOCUSING);
    }

    //adb by huangfei for front touch AE start
    @Override
    public void startFrontTouchAe() {
        if (getHandler() == null) {
            LogHelper.w(TAG, "getHandler = " +
                    getHandler());
            return;
        }
        getHandler().removeCallbacks(mDisappear);
        ...
        mFocusRing.setImageDrawable(this.getResources().getDrawable(R.drawable.bv_ic_touch_focus));
        setVisibility(VISIBLE);
        //animate().withLayer().setDuration(SCALING_UP_TIME).scaleX(1.2f).scaleY(1.2f).alpha(1.0f);	//bv wuyonglin detele for bug4492 20210329
        setFocusState(FocusViewState.STATE_ACTIVE_FOCUSING);
        //bv wuyonglin add for bug4492 20210329 start
        postDelayed(new ActiveFocusEndAction(), SCALING_DOWN_STAY_TIME);
        //bv wuyonglin add for bug4492 20210329 end
    }
    ...
    public void stopFocusAnimations() {
        if (isPassiveFocusRunning()) {
            ...
            //bv wuyonglin add for bug4492 20210329 start
            animate().withLayer().setDuration(SCALING_DOWN_TIME).scaleX(1f).scaleY(1f).setListener(
                    new Animator.AnimatorListener() {
                        ...
                        public void onAnimationEnd(Animator animator) {
                            LogHelper.i(TAG,"onAnimationEnd mExpandView.setVisibility(VISIBLE)");
                            mExpandView.setVisibility(VISIBLE);
                            animate().withLayer().setListener(null);
                        }
    ...
    private class ActiveFocusEndAction implements Runnable {
        ...
        public void run() {
            ...
            //*/ hct.huangfei, 20201111. manual focus show time.
            if(!highlightFocusView){
                postDelayed(mDisappear, SCALING_UP_TIME);
            }
            //*/ 
        }
    }
