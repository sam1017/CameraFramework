...
package com.mediatek.camera.feature.mode.mono;
...
import com.mediatek.camera.feature.mode.mono.device.MonoDeviceControllerFactory;
import com.mediatek.camera.feature.mode.mono.device.IMonoDeviceController;
import com.mediatek.camera.feature.mode.mono.device.IMonoDeviceController.DataCallbackInfo;
import com.mediatek.camera.feature.mode.mono.device.IMonoDeviceController.DeviceCallback;
import com.mediatek.camera.feature.mode.mono.device.IMonoDeviceController.CaptureDataCallback;
import com.mediatek.camera.feature.mode.mono.device.IMonoDeviceController.PreviewSizeCallback;


import com.mediatek.camera.feature.mode.mono.device.IMonoDeviceController.CaptureImageSavedCallback;
 ...
 */
public class MonoMode extends CameraModeBase implements CaptureDataCallback,
        DeviceCallback, PreviewSizeCallback, IMemoryManager.IMemoryListener,
        CaptureImageSavedCallback {
    private static final LogUtil.Tag TAG = new LogUtil.Tag(MonoMode.class.getSimpleName());
    private static final String KEY_MATRIX_DISPLAY_SHOW = "key_matrix_display_show";
    private static final String MODE_KEY = MonoMode.class.getName();
    ...
    protected IMonoDeviceController mIMonoDeviceController;
    protected MonoModeHelper mMonoModeHelper;
    ...
    public void init(@Nonnull IApp app, @Nonnull ICameraContext cameraContext,
        ...
        MonoDeviceControllerFactory deviceControllerFactory = new MonoDeviceControllerFactory();
        mIMonoDeviceController = deviceControllerFactory.createDeviceController(app.getActivity(),
        ...
        mMonoModeHelper = new MonoModeHelper(cameraContext);
        LogHelper.d(TAG, "[init]- ");
    }
    ...
    public void resume(@Nonnull DeviceUsage deviceUsage) {
        ...
        mIMonoDeviceController.queryCameraDeviceManager();
    ...
    public void pause(@Nullable DeviceUsage nextModeDeviceUsage) {
        ...
        if (mNeedCloseCameraIds.size() > 0) {
            ...
            mIMonoDeviceController.closeSession();
        } else{
            clearAllCallbacks(mCameraId);
            mIMonoDeviceController.stopPreview();
        }
        LogHelper.i(TAG, "[pause]-");
    }
    ...
    public void unInit() {
        super.unInit();
        mIMonoDeviceController.destroyDeviceController();
    }
    ...
    protected boolean doShutterButtonClick() {
        ...
        boolean isDeviceReady = mIMonoDeviceController.isReadyForCapture();
        ...
        if (storageReady && isDeviceReady && mIsResumed
            ...
            mIMonoDeviceController.updateGSensorOrientation(mIApp.getGSensorOrientation());
            mIMonoDeviceController.takePicture(this);
        ...
        if (mIsResumed) {
            ...
            if (mCameraApi == CameraApi.API1) {
                if (needRestartPreview && !mIsMatrixDisplayShow) {
                    mIMonoDeviceController.startPreview();
                }
            }
        }
        //update thumbnail
...
/*                HeifHelper heifHelper = new HeifHelper(mICameraContext);
    ...
    public void onCameraOpened(String cameraId) {
        ...
        Relation relation = MonoRestriction.getRestriction().getRelation("on", true);
    ...
    private void prepareAndOpenCamera(boolean needOpenCameraSync, String cameraId,
        ...
        mIMonoDeviceController.setDeviceCallback(this);
        mIMonoDeviceController.setPreviewSizeReadyCallback(this);
        mIMonoDeviceController.setSavedDataCallback(this);
        //prepare device info.
        MonoDeviceInfo info = new MonoDeviceInfo();
    ...
    private void clearAllCallbacks(String cameraId) {
        mIMonoDeviceController.setPreviewSizeReadyCallback(null);
    ...
    private void updatePictureSizeAndPreviewSize(Size previewSize) {
        ...
        if (size != null && mIsResumed) {
            ...
            mIMonoDeviceController.setPictureSize(new Size(mCaptureWidth, mCaptureHeight));
    ...
    private void saveData(byte[] data) {
        if (data != null) {
            ...
            ContentValues contentValues = mMonoModeHelper.createContentValues(data,
            ...
            mIMonoDeviceController.addExif(FilePath, data);
    ...
    private class SurfaceChangeListener implements ISurfaceStatusListener {
        ...
        public void surfaceAvailable(Object surfaceObject, int width, int height) {
            LogHelper.d(TAG, "surfaceAvailable,device controller = " + mIMonoDeviceController
                    + ",w = " + width + ",h = " + height);
            if (mModeHandler != null) {
                mModeHandler.post(new Runnable() {
                    @Override
                    public void run() {
            if (mIMonoDeviceController != null && mIsResumed) {
                mIMonoDeviceController.updatePreviewSurface(surfaceObject);
            }
        }
                });
            }
        ...
        public void surfaceChanged(Object surfaceObject, int width, int height) {
            LogHelper.d(TAG, "surfaceChanged, device controller = " + mIMonoDeviceController
                    + ",w = " + width + ",h = " + height);
            if (mModeHandler != null) {
                mModeHandler.post(new Runnable() {
                    @Override
        ...
        public void surfaceDestroyed(Object surfaceObject, int width, int height) {
            LogHelper.d(TAG, "surfaceDestroyed,device controller = " + mIMonoDeviceController);
        }
    }
    ...
    private class MyStatusChangeListener implements StatusChangeListener {
        ...
        public void onStatusChanged(String key, String value) {
            ...
            if (KEY_PICTURE_SIZE.equalsIgnoreCase(key)) {
                ...
                mIMonoDeviceController.setPictureSize(new Size(mCaptureWidth, mCaptureHeight));
                Size previewSize = mIMonoDeviceController.getPreviewSize((double) mCaptureWidth /
                        mCaptureHeight);
                int width = previewSize.getWidth();
                int height = previewSize.getHeight();
                if (width != mPreviewWidth || height != mPreviewHeight) {
                ...
                mIMonoDeviceController.setFormat(value);
