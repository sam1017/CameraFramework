...
/**
 * An implementation of {@link IPanormaDeviceController} with Camera2Proxy.
 */
@TargetApi(Build.VERSION_CODES.LOLLIPOP)
class PanoramaDeviceController extends Device2Controller implements
        IPanormaDeviceController,
        CaptureSurface.ImageCallback,
        ISettingManager.SettingDevice2Requester {
    private static final Tag TAG = new Tag(PanoramaDeviceController.class.getSimpleName());
    private static final String KEY_PICTURE_SIZE = "key_picture_size";
    private static final int CAPTURE_MAX_NUMBER = 1;
    ...
    private static final String MTK_POSTALGO_AUTORAMA_CMD = "postalgo.autorama.cmd";
    private static final String MTK_POSTALGO_AUTORAMA_MOTION_DATA = "postalgo.autorama.motion.data";
    private static final String POSTALGO_PARAMS_JPEG_ORIENTATION_KEY =
                                "postalgo.capture.jpegorientation";
    private static final int AUTORAMA_MAX_WIDTH = 30720;
    private int[] mPostalgoBack = new int[4];
    ...
    /**
     * PanoramaDeviceController may use activity to get display rotation.
     *
     * @param activity the camera activity.
     */
    PanoramaDeviceController(@Nonnull Activity activity, @Nonnull ICameraContext context) {
        LogHelper.d(TAG, "[PanoramaDeviceController]");
        mActivity = activity;
    ...
    public void updatePreviewSurface(Object surfaceObject) {
        LogHelper.d(TAG, "[updatePreviewSurface] surfaceHolder = " + surfaceObject +
                " state = " + mCameraState + ", session :" + mSession);
    ...
    public void mergePicture() {
        ...
        configMetaParams(MTK_POSTALGO_AUTORAMA_CMD, 1);
        configMetaParams(POSTALGO_PARAMS_JPEG_ORIENTATION_KEY, rotation);
    }

    @Override
    public void cancleMergePicture() {
        configMetaParams(MTK_POSTALGO_AUTORAMA_CMD, 0);
        mBuilder.removeTarget(mCapturePostAlgoSurface);
    ...
    public void closeCamera(boolean sync) {
        LogHelper.i(TAG, "[closeCamera] + sync = " + sync +
                " current state : " + mCameraState);
    ...
    public void createAndChangeRepeatingRequest() {
        LogHelper.i(TAG, "mCamera2Proxy= " + mCamera2Proxy + "mCameraState" + mCameraState);
    ...
    private void updateCameraState(CameraState state) {
        LogHelper.d(TAG, "[updateCameraState] new state = " + state +
                " old =" + mCameraState);
        mLockState.lock();
        try {
            mCameraState = state;
    ...
    private void initSettings() throws CameraAccessException {
        ...
        Relation relation = PanoramaRestriction.getRestriction().getRelation("on",
                false);
    ...
    private void startPostAlgo() {
        mCaptureSurface.updatePictureInfo(AUTORAMA_MAX_WIDTH,
                mPreviewHeight, ImageFormat.JPEG, CAPTURE_MAX_NUMBER);
        ...
        LogHelper.i(TAG, "[startPostAlgo] add capture surface");
        captureFeatureConfig.addSurface(captureSurfaceList);

        FeaturePipeConfig captureFeaturePipeConfig = new FeaturePipeConfig();
        captureFeaturePipeConfig.addFeaturePipeConfig(FeaturePipeConfig.INDEX_PREVIEW,
        ...
        captureStreamInfo.addInfo(mPreviewWidth, mPreviewHeight,
                ImageFormat.YUV_420_888, 0);
        LogHelper.i(TAG, "[startPostAlgo] add captureStreamInfo," +
        ...
        FeatureResult result = mICameraContext.getCamPostAlgo().start(featureConfigs,
                new ICamPostAlgoCallback.Stub() {
                    ...
                    public void processResult(FeatureParam featureParam) throws RemoteException {
                        featureParam.getIntArray(MTK_POSTALGO_AUTORAMA_MOTION_DATA, mPostalgoBack);
                        mCaptureDataCallback.onPostalgoReceived(mPostalgoBack[0], mPostalgoBack[1],
                                mPostalgoBack[2], mPostalgoBack[3]);
                    }
