...
public class PictureSizeHelper {
    ...
    //*/ hct.huangfei, 20201024. add picture size ratio(18:9).
    public static final double RATIO_18_9 = 18d / 9;
    //*/
    ...
    private static final double RATIOS[] = { RATIO_1_1,RATIO_18_9, RATIO_16_9, RATIO_5_3, RATIO_3_2, RATIO_4_3 };

    //*/ hct.huangfei, 20201024. add picture size ratio(18:9).
    private static final String RATIO_18_9_IN_STRING = "(18:9)";
    //*/
	//add by huangfei for  picture size ratio 1:1 start
    private static final String RATIO_1_1_IN_STRING = "(1:1)";
	//add by huangfei for  picture size ratio 1:1 end	
    ...
    private static final String RATIOS_IN_STRING[] = { RATIO_1_1_IN_STRING,RATIO_18_9_IN_STRING, RATIO_16_9_IN_STRING, RATIO_5_3_IN_STRING,
    ...
    public static double findFullScreenRatio(Context context) {
        ...
        for (int i = 0; i < RATIOS.length; i++) {
            ...
            //*/ hct.huangfei, 20201024. add picture size ratio(18:9).
            if (!Config.isCameraRatio18_9Support(context) && ratio == RATIO_18_9){
                continue;
            }
            //*/
            //add by huangfei for  picture size ratio 1:1 start
            if (!Config.isCameraRatio_1_1_Support(context) && ratio == RATIO_1_1){
                continue; 
            }
            //add by huangfei for  picture size ratio 1:1 end			
    ...
    public static List<String> filterSizes(List<String> sizes,Context context) {
        // create buckets.
        Map<Double, ResolutionBucket> buckets = new HashMap<>();
        for (double ratio : sDesiredAspectRatios) {
            ResolutionBucket bucket = new ResolutionBucket();
        ...
        // filter sizes from buckets.
        //*/ hct.huangfei, 20210106.promode supporte.
        int maxPixel = pixels(valueToSize(sizes.get(0)));
        if(maxPixel<=2){
            sMaxCount = 1;
        }else if(maxPixel<=5){
            sMaxCount = 2;
        ...
        //*/
        //int showNums = context.getResources().getInteger(R.integer.picture_size_numbers_show);
        List<Size> sizesAfterFilter = new LinkedList<>();
        for (double ratio : sDesiredAspectRatios) {
            ResolutionBucket bucket = buckets.get(ratio);
            if (bucket.sizes.size() == 0) {
            ...
            for (int i = 0; i < bucket.sizes.size(); i++) {
                if (i >= sMaxCount) {
                    break;
                }
                int index = -1;
                Size size = bucket.sizes.get(i);
                for (int j = 0; j < sizesAfterFilter.size(); j++) {
                    if (pixels(size) > pixels(sizesAfterFilter.get(j))) {
                        index = j;
                        break;
                    }
                }
        ...
        List<String> sizesInstr = new ArrayList<>();
        if (Config.pictureSizeShowModeSupport(context)){
            int size = sizesAfterFilter.size() > sDesiredAspectRatios.size() * sMaxCount
                    ? sDesiredAspectRatios.size() * sMaxCount : sizesAfterFilter.size();
            for (int i = 0; i < size; i++) {
                //HCT.ouyang pixels greater than 0M
                if (pixels(sizesAfterFilter.get(i)) > 0)
                    sizesInstr.add(sizeToStr(sizesAfterFilter.get(i)));
            }
        }else{
            for (Size size : sizesAfterFilter) {
                sizesInstr.add(sizeToStr(size));
    ...
    private static int pixels(Size size){
        ...
        return Integer.parseInt(sFormat.format(Math.round(size.width * size.height / 1e6)));
    }
    //*/
}
