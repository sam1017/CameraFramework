...
package com.mediatek.camera.feature.mode.hctfacebeauty;
...
import com.mediatek.camera.feature.mode.hctfacebeauty.device.IHctFaceBeautyDeviceController;
import com.mediatek.camera.feature.mode.hctfacebeauty.device.IHctFaceBeautyDeviceController.DataCallbackInfo;
import com.mediatek.camera.feature.mode.hctfacebeauty.device.IHctFaceBeautyDeviceController.DeviceCallback;
import com.mediatek.camera.feature.mode.hctfacebeauty.device.IHctFaceBeautyDeviceController.CaptureDataCallback;
import com.mediatek.camera.feature.mode.hctfacebeauty.device.IHctFaceBeautyDeviceController.PreviewSizeCallback;
import com.mediatek.camera.feature.mode.hctfacebeauty.device.HctFaceBeautyDeviceControllerFactory;
import com.mediatek.camera.feature.mode.hctfacebeauty.device.IHctFaceBeautyDeviceController.CaptureImageSavedCallback;
...
import com.mediatek.camera.feature.mode.hctfacebeauty.view.BeautyViewCtrl;
 ...
 */
public class HctFaceBeautyMode extends CameraModeBase implements CaptureDataCallback,
        DeviceCallback, PreviewSizeCallback, IMemoryManager.IMemoryListener,
        CaptureImageSavedCallback {
    private static final LogUtil.Tag TAG = new LogUtil.Tag(HctFaceBeautyMode.class.getSimpleName());
    ...
    protected IHctFaceBeautyDeviceController mIHctFaceBeautyDeviceController;
    protected HctFaceBeautyModeHelper mHctFaceBeautyModeHelper;
    ...
    public void init(@Nonnull IApp app, @Nonnull ICameraContext cameraContext,
        ...
        HctFaceBeautyDeviceControllerFactory deviceControllerFactory = new HctFaceBeautyDeviceControllerFactory();
        mIHctFaceBeautyDeviceController = deviceControllerFactory.createDeviceController(app.getActivity(),
        ...
        mHctFaceBeautyModeHelper = new HctFaceBeautyModeHelper(cameraContext);
        LogHelper.d(TAG, "[init]- ");
    ...
    public void resume(@Nonnull DeviceUsage deviceUsage) {
        ...
        mIHctFaceBeautyDeviceController.queryCameraDeviceManager();
    ...
    public void pause(@Nullable DeviceUsage nextModeDeviceUsage) {
        ...
        if (mNeedCloseCameraIds.size() > 0) {
            ...
            mIHctFaceBeautyDeviceController.closeSession();
        } else{
            clearAllCallbacks(mCameraId);
            mIHctFaceBeautyDeviceController.stopPreview();
        }
        LogHelper.i(TAG, "[pause]-");
    }
    ...
    public void unInit() {
        ...
        mIHctFaceBeautyDeviceController.destroyDeviceController();
    }
    ...
    protected boolean doShutterButtonClick() {
        ...
        boolean isDeviceReady = mIHctFaceBeautyDeviceController.isReadyForCapture();
        ...
        if (storageReady && isDeviceReady && mIsResumed
            ...
            mIHctFaceBeautyDeviceController.updateGSensorOrientation(mIApp.getGSensorOrientation());
            mIHctFaceBeautyDeviceController.takePicture(this); 
        }
        return true;
    }
    ...
    public void onDataReceived(DataCallbackInfo dataCallbackInfo) {
        ...
        if (mIsResumed) {
            ...
            if (mCameraApi == CameraApi.API1) {
                if (needRestartPreview && !mIsMatrixDisplayShow) {
                    mIHctFaceBeautyDeviceController.startPreview();
                }
            }
        }
        //update thumbnail
        if (data != null && needUpdateThumbnail) {
            if (format == ImageFormat.JPEG) {
                ...
                //modify by huangfei for update thumbnail without exif start
                //updateThumbnail(data);
                if(Config.updateThumbnailWithoutExif(mIApp.getActivity())){
                    updateThumbnail(data,mCaptureWidth);
                }else{
                    updateThumbnail(data);
                }
                //modify by huangfei for update thumbnail without exif end
            } else if (format == HeifHelper.FORMAT_HEIF) {
/*                HeifHelper heifHelper = new HeifHelper(mICameraContext);
    ...
    public void onCameraOpened(String cameraId) {
        ...
        Relation relation = HctFaceBeautyRestriction.getRestriction().getRelation("on", true);
    ...
    private void prepareAndOpenCamera(boolean needOpenCameraSync, String cameraId,
        ...
        mIHctFaceBeautyDeviceController.setDeviceCallback(this);
        mIHctFaceBeautyDeviceController.setPreviewSizeReadyCallback(this);
        mIHctFaceBeautyDeviceController.setSavedDataCallback(this);
        //prepare device info.
        HctFaceBeautyDeviceInfo info = new HctFaceBeautyDeviceInfo();
        ...
        mIHctFaceBeautyDeviceController.openCamera(info);
    }

    private void prePareAndCloseCamera(boolean needSync, String cameraId) {
        clearAllCallbacks(cameraId);
        mIHctFaceBeautyDeviceController.closeCamera(needSync);
    ...
    private void clearAllCallbacks(String cameraId) {
        mIHctFaceBeautyDeviceController.setPreviewSizeReadyCallback(null);
    ...
    private void updatePictureSizeAndPreviewSize(Size previewSize) {
        ...
        if (size != null && mIsResumed) {
            ...
            mIHctFaceBeautyDeviceController.setPictureSize(new Size(mCaptureWidth, mCaptureHeight));
    ...
    private void saveData(byte[] data) {
        if (data != null) {
            ...
            ContentValues contentValues = mHctFaceBeautyModeHelper.createContentValues(data,
    ...
    private class SurfaceChangeListener implements ISurfaceStatusListener {
        ...
        public void surfaceAvailable(Object surfaceObject, int width, int height) {
            LogHelper.d(TAG, "surfaceAvailable,device controller = " + mIHctFaceBeautyDeviceController
                    + ",w = " + width + ",h = " + height);
            if (mModeHandler != null) {
                mModeHandler.post(new Runnable() {
                    @Override
                    public void run() {
            if (mIHctFaceBeautyDeviceController != null && mIsResumed) {
                mIHctFaceBeautyDeviceController.updatePreviewSurface(surfaceObject);
            }
        }
                });
            }
        ...
        public void surfaceChanged(Object surfaceObject, int width, int height) {
            LogHelper.d(TAG, "surfaceChanged, device controller = " + mIHctFaceBeautyDeviceController
                    + ",w = " + width + ",h = " + height);
            if (mModeHandler != null) {
                mModeHandler.post(new Runnable() {
                    @Override
        ...
        public void surfaceDestroyed(Object surfaceObject, int width, int height) {
            LogHelper.d(TAG, "surfaceDestroyed,device controller = " + mIHctFaceBeautyDeviceController);
        }
    }
    ...
    private class MyStatusChangeListener implements StatusChangeListener {
        ...
        public void onStatusChanged(String key, String value) {
            ...
            if (KEY_PICTURE_SIZE.equalsIgnoreCase(key)) {
                ...
                mIHctFaceBeautyDeviceController.setPictureSize(new Size(mCaptureWidth, mCaptureHeight));
                Size previewSize = mIHctFaceBeautyDeviceController.getPreviewSize((double) mCaptureWidth /
                        mCaptureHeight);
                int width = previewSize.getWidth();
                int height = previewSize.getHeight();
                if (width != mPreviewWidth || height != mPreviewHeight) {
                ...
                mIHctFaceBeautyDeviceController.setFormat(value);
    ...
    private BeautyViewCtrl.ViewChangeListener mViewChangeListener
        ...
        public void onBeautyLevelChanged(int smooth,int lighten,int ruddy,int slender) {
            mIHctFaceBeautyDeviceController.setBeautyParameter(smooth,lighten,ruddy,slender);
        }
