...
public class PictureSizeCaptureRequestConfig implements ICameraSetting.ICaptureRequestConfigure {
    ...
    private final String PHOTO_MODE = "Photo";
    ...
    public PictureSizeCaptureRequestConfig(PictureSize pictureSize,
        ...
        mMaxTextureSize = PictureSizeHelper.getMaxTexureSize();
        LogHelper.i(TAG, "[PictureSizeCaptureRequestConfig], final mMaxTextureSize:" + mMaxTextureSize);
    }
    ...
    private List<String> sizeToStr(List<Size> sizes) {
        ...
        for (Size size : sizes) {
            if (mPictureSize.getCurrentMode() != null) { //bv wuyonglin modify for PictureSize quickSwitch 20191224 start
            //add by huangfei for  picture size ratio 1:1 start
            //bv wuyonglin modify for PictureSize quickSwitch 202004008 start
            if(!PHOTO_MODE.equals(mPictureSize.getCurrentMode()) && !"Mono".equals(mPictureSize.getCurrentMode())
                    && !"Pro".equals(mPictureSize.getCurrentMode()) && !"PiBokeh".equals(mPictureSize.getCurrentMode())
                    && !"AiWorksBokeh".equals(mPictureSize.getCurrentMode())
                    && !"AiWorksBokehColor".equals(mPictureSize.getCurrentMode())
                    && !"Night".equals(mPictureSize.getCurrentMode())){
            //bv wuyonglin modify for PictureSize quickSwitch 202004008 end
                if(isRATIO_1_1(size) && !"AiworksFaceBeauty".equals(mPictureSize.getCurrentMode())){
                    continue;
                }
            } //bv wuyonglin modify for PictureSize quickSwitch 20191224 end
            //add by huangfei for  picture size ratio 1:1 end

            //HCT:ouyang remove 1920x1088 picture size begin
            if(size.getWidth() == 1920 && size.getHeight() == 1080) {
                continue;
            }
            //HCT:ouyang end
            //if("AiworksFaceBeauty".equals(mPictureSize.getCurrentMode())){
            //bv wuyonglin add for Superphoto should fixed picture size 20201023 start
            if((0 == mPictureSize.getCameraId() && !"Superphoto".equals(mPictureSize.getCurrentMode())) && (size.getWidth() > mMaxTextureSize || size.getHeight() > mMaxTextureSize)) {
	    ...
	    //}
            ...
            if (0 == mPictureSize.getCameraId() && "com.mediatek.camera.common.mode.photo.intent.IntentPhotoMode".equals(mPictureSize.getModeKey())
                    && (size.getWidth() > mMaxTextureSize || size.getHeight() > mMaxTextureSize)) {
                continue;
            }
    ...
    //add by huangfei for  picture size ratio 1:1 start
    public boolean isRATIO_1_1(Size size){
