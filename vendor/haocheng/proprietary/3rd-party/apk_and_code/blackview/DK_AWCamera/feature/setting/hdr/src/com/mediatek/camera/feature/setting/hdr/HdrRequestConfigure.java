...
public class HdrRequestConfigure implements ICameraSetting.ICaptureRequestConfigure, IHdr.Listener {
    ...
    public void setCameraCharacteristics(CameraCharacteristics characteristics) {
        ...
        //bv wuyonglin modify for front camera can not support hdr 20200107 start
		//bv liangchangwei add for add aiworks hdr support 20200708 start
		...
		//bv liangchangwei add for add aiworks hdr support 20200708 end
        //bv wuyonglin modify for front camera can not support hdr 20200107 end
    ...
    public void configCaptureRequest(CaptureRequest.Builder captureBuilder) {
        ...
        //bv wuyonglin detele for add hdr quickswitch 20191231 start
        /*int controlMode;
        ...
        //bv wuyonglin detele for add hdr quickswitch 20191231 end
    }
    ...
    public void onHdrValueChanged() {
        ...
        if ("com.mediatek.camera.common.mode.photo.PhotoMode".equals(mHdr.getCurrentModeKey())
                || "com.mediatek.camera.common.mode.photo.intent.IntentPhotoMode".equals(mHdr.getCurrentModeKey())) {
        //bv wuyonglin add for add hdr quickswitch 20191231 start
        LogHelper.d(TAG, "[onHdrValueChanged], to createAndChangeRepeatingRequest");
        mSettingDevice2Requester.requestRestartSession();
        //bv wuyonglin add for add hdr quickswitch 20191231 end
        mSettingDevice2Requester.createAndChangeRepeatingRequest();
        }else if("com.mediatek.camera.common.mode.video.VideoMode".equals(mHdr.getCurrentModeKey())){
            LogHelper.d(TAG, "[onHdrValueChanged], VideoMode to createAndChangeRepeatingRequest");
            mSettingDevice2Requester.createAndChangeRepeatingRequest();
        }
		 //add by liangchangwei for Aiworks Video HDR
        ...
        public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request,
            ...
            for (CaptureResult.Key<?> key : keyList) {
                if (key.getName().equals(HDR_KEY_DETECTION_RESULT)) {
                    ...
                    if (value != null) {
                    LogHelper.d(TAG, "onCaptureCompleted, value.length : " + value.length);
                    if(value.length == 0){
                        return;
                    }
    ...
    private void initHdrVideoSettingValues(CameraCharacteristics cs) {
        ...
        if(CameraUtil.MTKCAM_AIWORKS_VIDEO_HDR_SUPPORT && !CameraUtil.is_videoHdr_Force){
            ...
            if (supportVendorList != null) {
                for (int value : supportVendorList) {
                    LogHelper.d(TAG, "video support value: " + value + " CAM_HDR_FEATURE_HDR_MODE_VIDEO_ON[0] = " + CAM_HDR_FEATURE_HDR_MODE_VIDEO_ON[0]);
                    if (value == CAM_HDR_FEATURE_HDR_MODE_VIDEO_ON[0]) {
                        supportedList.add(HDR_ON_VALUE);
                        mIsVendorHdrSupported = true;
                    } else if (value == CAM_HDR_FEATURE_HDR_MODE_VIDEO_AUTO[0]) {
