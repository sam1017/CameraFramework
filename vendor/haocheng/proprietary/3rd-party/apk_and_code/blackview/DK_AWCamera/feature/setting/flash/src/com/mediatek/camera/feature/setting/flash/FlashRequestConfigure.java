...
public class FlashRequestConfigure implements ICameraSetting.ICaptureRequestConfigure {
    ...
    //add by huangfei for flash of torch mode start
    private boolean mTorchSupport = false;
    //add by huangfei for flash of torch mode end 

    //bv wuyonglin add for flash always bright after open hdr flash still always bright 20200104 start
    private boolean mHdrOpenFlag = false;
     ...
     */
	//modify by huangfei for flash default start
    /*public FlashRequestConfigure(Flash flash,
                                 ISettingManager.SettingDevice2Requester requester) {*/
    public FlashRequestConfigure(Context context, Flash flash,
                                 ISettingManager.SettingDevice2Requester requester) {
    mContext = context;
	//modify by huangfei for flash default start
        mFlash = flash;
    ...
    public void setCameraCharacteristics(CameraCharacteristics characteristics) {
        ...
        //add by huangfei for flash default start
        mFlash.initializeValue(mFlash.getSupportedPlatformValues(),
                mContext.getResources().getString(R.string.flash_default_value));
        //add by huangfei for flash default end
    ...
    private void initPlatformSupportedValues() {
        ...
        if (mIsFlashSupported || mIsPanelFlashSupported) {
            ...
            supportedList.add(Flash.FLASH_TORCH_VALUE);
        }
    ...
    private void initSettingEntryValues() {
        ...
        List<String> supportedList = new ArrayList<>();
	if (ICameraMode.ModeType.VIDEO != mFlash.getCurrentModeType()) {
        ...
        supportedEntryList.add(Flash.FLASH_TORCH_VALUE);
	} else {
        supportedEntryList.add(Flash.FLASH_OFF_VALUE);
        supportedEntryList.add(Flash.FLASH_TORCH_VALUE);
	}
    ...
    private void updateFlashMode() {
        ...
        if (Flash.FLASH_TORCH_VALUE.equalsIgnoreCase(mFlash.getValue())) {
            //bv wuyonglin add for flash always bright after open hdr flash still always bright 20200104 start
LogHelper.d(TAG, "[updateFlashMode] mNeedChangeFlashModeToTorch ="+mNeedChangeFlashModeToTorch+" mFlash.getCurrentModeType() =" +mFlash.getCurrentModeType()+" mHdrOpenFlag ="+mHdrOpenFlag);
	    if (!mHdrOpenFlag) {
            //bv wuyonglin add for flash always bright after open hdr flash still always bright 20200104 end
            mFlashMode = CameraMetadata.FLASH_MODE_TORCH;
            //bv wuyonglin add for flash always bright after open hdr flash still always bright 20200104 start
