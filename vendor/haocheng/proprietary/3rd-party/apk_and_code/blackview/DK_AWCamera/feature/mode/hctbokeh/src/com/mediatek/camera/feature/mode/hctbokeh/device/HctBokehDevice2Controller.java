...
package com.mediatek.camera.feature.mode.hctbokeh.device;
...
import com.mediatek.camera.feature.mode.hctbokeh.HctBokehDeviceInfo;
...
class HctBokehDevice2Controller extends Device2Controller implements
        IHctBokehDeviceController, ICompeletedCallback,
        CaptureSurface.ImageCallback,
        ISettingManager.SettingDevice2Requester {
    private static final Tag TAG = new Tag(HctBokehDevice2Controller.class.getSimpleName());
    ...
    private static final String MTK_CONTROL_CAPTURE_BRIGHT_VALUE = "com.mediatek.control.capture.hctbrightvalue";
    ...
    private boolean mt6580PlatForm = false;
     ...
     */
    HctBokehDevice2Controller(@Nonnull Activity activity, @Nonnull ICameraContext context) {
        LogHelper.d(TAG, "[HctBokehDevice2Controller]");
        mActivity = activity;
        ...
        mDefaultBokehLevel = mActivity.getResources().getInteger(R.integer.bokeh_level_default);
    ...
    @Override
    public void openCamera(HctBokehDeviceInfo info) {
        synchronized (CameraApplication.class) {
        CameraSysTrace.onEventSystrace("HctBokehDevice.openCamera", true, true);
        ...
        CameraSysTrace.onEventSystrace("HctBokehDevice.openCamera", false, true);
        }
    }
    ...
    @Override
    public void takePicture(@Nonnull IHctBokehDeviceController.CaptureDataCallback callback) {
        LogHelper.i(TAG, "[takePicture] mSession= " + mSession);
        CameraSysTrace.onEventSystrace("HctBokehDevice.takePicture", true, true);
        ...
        CameraSysTrace.onEventSystrace("HctBokehDevice.takePicture", false, true);
    }
    ...
    public void closeCamera(boolean sync) {
        ...
        CameraSysTrace.onEventSystrace("HctBokehDevice.closeCamera", true, true);
        ...
        mCurrentCameraId = null;
        CameraSysTrace.onEventSystrace("HctBokehDevice.closeCamera", false, true);
    ...
    public void onPictureCallback(byte[] data,
        ...
        if (mCaptureDataCallback != null) {
            ...
            if (ThumbnailHelper.isPostViewSupported()) {
                ...
                //delete by huangfei for Thumbnail update by post view start
                //info.needUpdateThumbnail = false;
                //delete by huangfei for Thumbnail update by post view end

            }         
            if (ThumbnailHelper.CAPTURE_TYPE_THUMBNAIL.equalsIgnoreCase(captureType)) {
                CameraSysTrace.onEventSystrace("HctBokehDevice.onPostViewCallback", true, true);

                //delete by huangfei for Thumbnail update by post view start
                //mCaptureDataCallback.onPostViewCallback(data);
                //delete by huangfei for Thumbnail update by post view end

                CameraSysTrace.onEventSystrace("HctBokehDevice.onPostViewCallback", false, true);
            } else {
                CameraSysTrace.onEventSystrace("HctBokehDevice.onJpegCallback", true, true);
                mCaptureDataCallback.onDataReceived(info);
                CameraSysTrace.onEventSystrace("HctBokehDevice.onJpegCallback", false, true);
    ...
    private void repeatingPreview(boolean needConfigBuiler) {
        ...
        CameraSysTrace.onEventSystrace("HctBokehDevice.repeatingPreview", true, true);
        if (mSession != null && mCamera2Proxy != null) {
            try {
                if (needConfigBuiler) {
        ...
        CameraSysTrace.onEventSystrace("HctBokehDevice.repeatingPreview", false, true);
    }
    ...
    private Builder doCreateAndConfigRequest(int templateType) throws CameraAccessException {
        ...
        if (progress== -1){
            ...
            mDataStore.setValue("hct_bokeh_progress",mbokehLevel+"",mDataStore.getGlobalScope(),false);
        }else{
    ...
    private final CaptureCallback mCaptureCallback = new CaptureCallback() {
        ...
        public void onCaptureProgressed(CameraCaptureSession session, CaptureRequest request,
            ...
            if (CameraUtil.isStillCaptureTemplate(request)
                ...
                CameraSysTrace.onEventSystrace("HctBokehDevice.onP2Done", true, true);
                LogHelper.d(TAG, "[onCaptureProgressed] P2done comes, frame: " + num);
                updateCameraState(CameraState.CAMERA_OPENED);
                mModeDeviceCallback.onPreviewCallback(null, 0);
                CameraSysTrace.onEventSystrace("HctBokehDevice.onP2Done", false, true);
            }
        }
        ...
        public void onCaptureCompleted(@Nonnull CameraCaptureSession session,
            ...
            if (CameraUtil.isStillCaptureTemplate(result)) {
                ...
                CameraSysTrace.onEventSystrace("HctBokehDevice.onFirstFrameArrived", true, true);
                mFirstFrameArrived = true;
                updateCameraState(CameraState.CAMERA_OPENED);
                mModeDeviceCallback.onPreviewCallback(null, 0);
                mICameraContext.getSoundPlayback().init();
                CameraSysTrace.onEventSystrace("HctBokehDevice.onFirstFrameArrived", false, true);
            }
    ...
    public void setBokehParameter(int level,int x,int y,int r) {
        ...
        bokeh_y = y;
        mRadius = r;
        mDataStore.setValue("hct_bokeh_progress",level+"",mDataStore.getGlobalScope(),false);
        createAndChangeRepeatingRequest();
    }

    public int getBokehLevel(){
        int progress = Integer.parseInt(mDataStore.getValue("hct_bokeh_progress",mDefaultBokehLevel+"",mDataStore.getGlobalScope()));
        return progress;
    }
    ...
    private void setBokehParameter(CaptureRequest.Builder builder,boolean enable){
        ...
        CaptureRequest.Key<int[]> mBokehmode =  new CaptureRequest.Key<int[]>("com.mediatek.control.capture.hcttripartitealgorithmmode", int[].class);
        CaptureRequest.Key<int[]> mBokehLevel =  new CaptureRequest.Key<int[]>("com.mediatek.control.capture.hctbokehparameter", int[].class);
        BOKEH_MODE_VALUE[0] = 256;
        BOKEH_MODE_VALUE[1] = 256;
    ...
    private void hct_set_bright_as_polling_mode() {
        if(!mICameraContext.getIApp().getAppUi().getCurrentMode().equals("HctBokeh")){
            return;
        }
        ...
        int reg4 = 0;
        int level = 0;
        mt6580PlatForm = "mt6580".equals(android.os.SystemProperties.get("ro.board.platform"));
        if(mt6580PlatForm){
            try{
                String value = android.os.SystemProperties.get("vendor.hct.camera.bv");
                maincam_bright = Integer.parseInt(value);
