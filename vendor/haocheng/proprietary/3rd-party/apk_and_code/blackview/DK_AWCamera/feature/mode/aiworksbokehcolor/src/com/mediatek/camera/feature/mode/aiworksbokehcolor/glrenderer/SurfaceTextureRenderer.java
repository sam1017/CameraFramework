...
public class SurfaceTextureRenderer implements OnFrameAvailableListener {
    ...
    private PortraitDrawer mPortraitDrawer;
    ...
    private Runnable mRenderTask = new Runnable() {
        ...
        //Log.i(TAG, "mRenderTask to drawFrame mEglDisplay ="+mEglDisplay+" mEglSurface ="+mEglSurface);
                if (mEglDisplay != null && mEglSurface != null && mSurfaceTexture != null) {
		    try {
                        mSurfaceTexture.updateTexImage();
                        float[] mTransform = new float[16];
                        mSurfaceTexture.getTransformMatrix(mTransform);
                        mPortraitDrawer.onDrawFrame(mTransform, mPreviewWidth, mPreviewHeight, mPreviewWidth, mPreviewHeight);
    ...
    public SurfaceTextureRenderer(TextureView textureView, PortraitEffect effect, SurfaceTexture target) {
        mPortraitDrawer = new PortraitDrawer(effect);
        mTextureView = textureView;

        mEglThread = new HandlerThread("SurfaceTextureRenderer");
        mEglThread.start();
        mEglHandler = new Handler(mEglThread.getLooper());
        Log.d(TAG, "SurfaceTextureRenderer initialize target ="+target+" mSurfaceTexture ="+mSurfaceTexture);
        initialize(target);
    }
    ...
    public void acquireSurfaceTexture(final int previewWidth, final int previewHeight, final SurfaceTextureListener surfaceTextureListener) {
        ...
        mEglHandler.post((new Runnable() {
            @Override
            public void run() {
                mSurfaceTexture = mPortraitDrawer.createSurfaceTexture();
       Log.d(TAG, "acquireSurfaceTexture onSurfaceTextureAvailable setDefaultBufferSize1 previewHeight ="+previewHeight+" previewWidth ="+previewWidth);
        ...
        waitDone();
        Log.i(TAG, "acquireSurfaceTexture waitDone over");
    }

    public void releaseSurfaceTexture() {
        Log.i(TAG, "releaseSurfaceTexture123");
        mEglHandler.post((new Runnable() {
            @Override
            public void run() {
                ...
                mPortraitDrawer.releaseSurfaceTexture();
            }
        }));
        waitDone();
        Log.i(TAG, "releaseSurfaceTexture waitDone over");
    }

    public void releaseSurfaceTexture(boolean isDestroy) {
        Log.i(TAG, "releaseSurfaceTexture123  isDestroy");
        mEglHandler.post((new Runnable() {
            @Override
            public void run() {
                if (mSurfaceTexture != null) {
                ...
                mPortraitDrawer.releaseSurfaceTexture(isDestroy);
            }
        }));
    }
