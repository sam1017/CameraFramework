...
VendorFaceBeautyDeviceController extends Device2Controller implements
    ...
    private static final int PREVIEW_SIZE_HEIGHT = 720;
    ...
    private static final String MTK_POSTALGO_VENDOR_FACE_BEAUTY_FACEORIENTATION
            = "postalgo.vfacebeauty.faceorientation";
    ...
    private Handler mFeatureHandler;
    ...
    /**
     * VendorFaceBeautyDeviceController may use activity to get display rotation.
     *
    ...
    VendorFaceBeautyDeviceController(@Nonnull Activity activity, @Nonnull ICameraContext context) {
        ...
        mAccelerometer = new Accelerometer(mActivity);
        mAccelerometer.setDirectionListener(mDirectionListener);
        mFeatureHandler = mModeHandler;
    }
    ...
    private Size getTargetPreviewSize(double ratio) {
        ...
        try {
            ...
            if (sizes.size() - sizes.indexOf(values) > 0) {
                ...
                //Control size below 720P
                while (values.getHeight() > PREVIEW_SIZE_HEIGHT) {
                    values = sizes.get(sizes.indexOf(values) + 1);
                }
            }
    ...
    private void initSettings() {
        ...
        mSettingController.postRestriction(VendorFaceBeautyRestriction.getRestriction()
    ...
    private void startPostAlgo() {
        ...
        int faceOrientation = FaceOrientationUtil.dupFaceOrientation(mPhoneSensorOrientation,
                facing == LENS_FACING_FRONT);
        mFeatureParam.appendInt(MTK_POSTALGO_VENDOR_FACE_BEAUTY_FACEORIENTATION,
                faceOrientation);
        initMetaParams();
    ...
    private IDirectionListener mDirectionListener = new IDirectionListener() {
        @Override
        public void onDirectionChanged(int direction) {
            if (mPhoneSensorOrientation != direction) {
                mPhoneSensorOrientation = direction;
                setFaceOrientation(mPhoneSensorOrientation);
            }

        }
    };

    private void setFaceOrientation(int direction) {
        mFeatureHandler.post(new Runnable() {
            @Override
            public void run() {
                int facing = getCameraFacingById(mCurrentCameraId);
                int faceOrientation = FaceOrientationUtil.dupFaceOrientation(direction,
                        facing == LENS_FACING_FRONT);
                mFeatureParam = new FeatureParam();
                mFeatureParam.appendInt(MTK_POSTALGO_VENDOR_FACE_BEAUTY_FACEORIENTATION,
                        faceOrientation);
                ...
                mICameraContext.getCamPostAlgo().configParams(FeaturePipeConfig.INDEX_CAPTURE,
                        mFeatureParam);
                LogHelper.e(TAG,
                        "sensorOritentation:" + direction + " faceOrientation = " + faceOrientation);
            }
        });
