...
import com.mediatek.camera.feature.setting.format.Format;
...
public class ContinuousShot2 extends ContinuousShotBase implements ICaptureRequestConfigure,
    ...
    public void overrideValues(@Nonnull String headerKey, String currentValue,
        ...
        if (headerKey.equals("key_hdr")) {
            //bv wuyonglin add modify after hdr open ,exit camera then open camera close hdr can not continuous shot and switch to front camera long press shutter button happen exception 20200302 start
            String hdrValue = mDataStore.getValue("key_hdr", HDR_DEFAULT_VALUE, mDataStore.getGlobalScope());
            LogHelper.i(TAG, "[overrideValues] hdrValue ="+hdrValue);
            ...
            if (hdrValue.equals("off") && !mSettingController.getCameraId().equals("1") && "com.mediatek.camera.common.mode.photo.PhotoMode".equals(getCurrentModeKey())) {
            ...
            //bv wuyonglin add modify after hdr open ,exit camera then open camera close hdr can not continuous shot and switch to front camera long press shutter button happen exception 20200302 end
            //bv wuyonglin add for after hdr open slide to pro mode then back to photo mode close hdr enter to setting should show continuous shot setting view 20200314 start
                if (!CameraUtil.isWaterMarkOn(mDataStore, mApp.getActivity())) {
        ...
        //bv wuyonglin add for after hdr open slide to pro mode then back to photo mode close hdr enter to setting should show continuous shot setting view 20200314 start
        if (headerKey.equals("key_water_mark") && (mSettingController.getCameraId().equals("0")|| mSettingController.getCameraId().equals("2"))&& currentValue != null && currentValue.equals("on")
		&& "com.mediatek.camera.common.mode.photo.PhotoMode".equals(getCurrentModeKey())
		&& (mDataStore.getValue("key_hdr", HDR_DEFAULT_VALUE, mDataStore.getGlobalScope()).equals("off"))) {
            mIsCshotSupported = true;
        }
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        mApp.getAppUi().updateCsState(true);
                        //*/
                        
                        onContinuousShotStarted();
                        createCaptureRequest(true);
                        //delete by huangfei for continuousshot play sound delay start
                        //playSound();
                        //delete by huangfei for continuousshot play sound delay end
    ...
    //*/ hct.huangfei, 20201027. getCsState.
    mApp.getAppUi().updateCsState(false);
    //*/
    ...
    private void createCaptureRequest(boolean isFirstRequest)
        ...
        if (mCaptureRequestNumber >= maxNuM) { 
        //*/
        ...
        //sharedCaptureSurface.setCaptureCallback(mImageCallback,1);
        mCaptureSurface.setCaptureCallback(mImageCallback);
        //*/
    ...
    private CaptureSurface.ImageCallback mImageCallback = new CaptureSurface.ImageCallback() {
        ...
        public void onPictureCallback(
            ...
            synchronized (mNumberLock) {
                if (data != null) {
                    //add by huangfei for continuousshot play sound delay start
                    //if(mImageCallbackNumber==0){
                        //playSound();
                    //}
                    //add by huangfei for continuousshot play sound delay end
                    ...
                    if (mImageCallbackNumber >= maxNuM && mCaptureSurface != null) {
                    //*/
