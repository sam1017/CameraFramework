...
class HdrDevice2Controller extends Device2Controller implements
        IHdrDeviceController,
        CaptureSurface.ImageCallback,
        ISettingManager.SettingDevice2Requester {
    private static final Tag TAG = new Tag(HdrDevice2Controller.class.getSimpleName());
     ...
     */
    HdrDevice2Controller(@Nonnull Activity activity, @Nonnull ICameraContext context) {
    ...
    private void configPostAlgo() {
        LogHelper.i(TAG, "[configPostAlgo] +");
        ...
        captureFeaturePipeConfig.addFeaturePipeConfig(FeaturePipeConfig.INDEX_CAPTURE,
                new int[]{FeaturePipeConfig.INDEX_HDR});
        captureFeatureConfig.addFeaturePipeConfig(captureFeaturePipeConfig);

        StreamInfo captureStreamInfo = new StreamInfo();
        captureStreamInfo.addInfo(getPictureSize().getWidth(), getPictureSize().getHeight(),
        ...
        int[] sizeArray = new int[]{getPictureSize().getWidth(), getPictureSize().getHeight()};
        featureParam.appendIntArray("postalgo.picturesize", sizeArray);
        captureFeatureConfig.addInterfaceParams(featureParam);
        ...
        mICameraContext.getCamPostAlgo().configParams(FeaturePipeConfig.INDEX_CAPTURE, featureParam);
        LogHelper.i(TAG, "[configPostAlgo] mCapturePostAlgoSurface = " + mCapturePostAlgoSurface);
    }
