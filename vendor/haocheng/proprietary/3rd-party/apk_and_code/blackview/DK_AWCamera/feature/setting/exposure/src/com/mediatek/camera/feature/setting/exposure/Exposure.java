...
public class Exposure extends SettingBase implements ExposureView.ExposureViewChangedListener,
    ...
    //*/ hct.huangfei, 20210107.zsd off after capture.
    public boolean isTribleCameraId(){
        String wideCameraId = SystemProperties.get("ro.hct_wide_angle_id","-1");
        String macroCameraId = SystemProperties.get("ro.hct_macroLens_id","-1");
        String nightCameraId = SystemProperties.get("ro.hct_night_vision_id","-1");
        String currentCameraId = mSettingController.getCameraId();
        if(currentCameraId.equals(wideCameraId) || currentCameraId.equals(macroCameraId)
                || currentCameraId.equals(nightCameraId)){
                    return true;
