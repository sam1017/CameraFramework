...
package com.mediatek.camera.feature.mode.mono.device;
...
import com.mediatek.camera.feature.mode.mono.MonoDeviceInfo;
...
import com.mediatek.camera.feature.mode.mono.MonoRestriction;
...
class MonoDevice2Controller extends Device2Controller implements
        IMonoDeviceController, ICompeletedCallback,
        CaptureSurface.ImageCallback,
        ISettingManager.SettingDevice2Requester {
    private static final Tag TAG = new Tag(MonoDevice2Controller.class.getSimpleName());
     ...
     */
    MonoDevice2Controller(@Nonnull Activity activity, @Nonnull ICameraContext context) {
        LogHelper.d(TAG, "[MonoDevice2Controller]");
        mActivity = activity;
    ...
    @Override
    public void openCamera(MonoDeviceInfo info) {
        synchronized (CameraApplication.class) {
        CameraSysTrace.onEventSystrace("MonoDevice.openCamera", true, true);
        ...
        CameraSysTrace.onEventSystrace("MonoDevice.openCamera", false, true);
        }
    }
    ...
    @Override
    public void takePicture(@Nonnull IMonoDeviceController.CaptureDataCallback callback) {
        LogHelper.i(TAG, "[takePicture] mSession= " + mSession);
        CameraSysTrace.onEventSystrace("MonoDevice.takePicture", true, true);
        ...
        CameraSysTrace.onEventSystrace("MonoDevice.takePicture", false, true);
    }
    ...
    public void closeCamera(boolean sync) {
        ...
        CameraSysTrace.onEventSystrace("MonoDevice.closeCamera", true, true);
        ...
        mCurrentCameraId = null;
        CameraSysTrace.onEventSystrace("MonoDevice.closeCamera", false, true);
...
*/
            ...
            if (ThumbnailHelper.CAPTURE_TYPE_THUMBNAIL.equalsIgnoreCase(captureType)) {
                CameraSysTrace.onEventSystrace("MonoDevice.onPostViewCallback", true, true);
                mCaptureDataCallback.onPostViewCallback(data);
                CameraSysTrace.onEventSystrace("MonoDevice.onPostViewCallback", false, true);
            } else {
                CameraSysTrace.onEventSystrace("MonoDevice.onJpegCallback", true, true);
                mCaptureDataCallback.onDataReceived(info);
                CameraSysTrace.onEventSystrace("MonoDevice.onJpegCallback", false, true);
    ...
    private void configureQuickPreview(Builder builder) {
        ...
        //add by huangei for disable quick preview of monomode start
        if(true){
            return;
        }
        //add by huangei for disable quick preview of monomode end
    ...
    private void configurePlatformCamera(Builder builder) {
        if (mCurrentCameraId != null) {
            ...
            if (keyPlatformCamera != null) {
                ...
                //add by huangfei for mono does not works when manual focus start
                builder.set(CaptureRequest.CONTROL_EFFECT_MODE, CameraMetadata.CONTROL_EFFECT_MODE_MONO);
                //add by huangfei for mono does not works when manual focus end
    ...
    private void repeatingPreview(boolean needConfigBuiler) {
        ...
        CameraSysTrace.onEventSystrace("MonoDevice.repeatingPreview", true, true);
        if (mSession != null && mCamera2Proxy != null) {
            try {
                if (needConfigBuiler) {
                    ...
                    mBuilder.set(CaptureRequest.CONTROL_EFFECT_MODE, CameraMetadata.CONTROL_EFFECT_MODE_MONO);
        ...
        CameraSysTrace.onEventSystrace("MonoDevice.repeatingPreview", false, true);
    }
    ...
    private Builder doCreateAndConfigRequest(int templateType) throws CameraAccessException {
        ...
        if (mCamera2Proxy != null) {
            ...
            if (Camera2Proxy.TEMPLATE_PREVIEW == templateType) {
                ...
                //add by huangfei for mono does not work when manual focus start
                builder.set(CaptureRequest.CONTROL_EFFECT_MODE, CameraMetadata.CONTROL_EFFECT_MODE_MONO);
                //add by huangfei for mono does not work when manual focus end
    ...
    private final CaptureCallback mCaptureCallback = new CaptureCallback() {
        ...
        public void onCaptureProgressed(CameraCaptureSession session, CaptureRequest request,
            ...
            if (CameraUtil.isStillCaptureTemplate(request)
                ...
                CameraSysTrace.onEventSystrace("MonoDevice.onP2Done", true, true);
                LogHelper.d(TAG, "[onCaptureProgressed] P2done comes, frame: " + num);
                updateCameraState(CameraState.CAMERA_OPENED);
                mModeDeviceCallback.onPreviewCallback(null, 0);
                CameraSysTrace.onEventSystrace("MonoDevice.onP2Done", false, true);
            }
        }
        ...
        public void onCaptureCompleted(@Nonnull CameraCaptureSession session,
            ...
            if (CameraUtil.isStillCaptureTemplate(result)) {
                ...
                CameraSysTrace.onEventSystrace("MonoDevice.onFirstFrameArrived", true, true);
                mFirstFrameArrived = true;
                updateCameraState(CameraState.CAMERA_OPENED);
                mModeDeviceCallback.onPreviewCallback(null, 0);
                mICameraContext.getSoundPlayback().init();
                CameraSysTrace.onEventSystrace("MonoDevice.onFirstFrameArrived", false, true);
            }
