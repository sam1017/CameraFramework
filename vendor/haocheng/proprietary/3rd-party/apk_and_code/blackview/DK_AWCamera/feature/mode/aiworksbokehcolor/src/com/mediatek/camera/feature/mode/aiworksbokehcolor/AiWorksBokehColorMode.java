...
package com.mediatek.camera.feature.mode.aiworksbokehcolor;
...
import com.mediatek.camera.feature.mode.aiworksbokehcolor.device.IAiWorksBokehColorDeviceController;
import com.mediatek.camera.feature.mode.aiworksbokehcolor.device.IAiWorksBokehColorDeviceController.DataCallbackInfo;
import com.mediatek.camera.feature.mode.aiworksbokehcolor.device.IAiWorksBokehColorDeviceController.DeviceCallback;
import com.mediatek.camera.feature.mode.aiworksbokehcolor.device.IAiWorksBokehColorDeviceController.CaptureDataCallback;
import com.mediatek.camera.feature.mode.aiworksbokehcolor.device.IAiWorksBokehColorDeviceController.PreviewSizeCallback;
import com.mediatek.camera.feature.mode.aiworksbokehcolor.device.AiWorksBokehColorDeviceControllerFactory;
...
import com.mediatek.camera.common.relation.Relation;
import com.mediatek.camera.feature.mode.aiworksbokehcolor.AiWorksBokehColorRestriction;
import com.mediatek.camera.feature.mode.aiworksbokehcolor.glrenderer.SurfaceTextureRenderer;
import com.aiworks.android.portrait.PortraitEffect;
import android.os.Environment;
import android.graphics.SurfaceTexture;
import com.mediatek.camera.feature.mode.aiworksbokehcolor.glrenderer.SurfaceTextureListener;
...
import com.mediatek.camera.feature.mode.aiworksbokehcolor.util.AiWorksUtil;
import com.mediatek.camera.feature.mode.aiworksbokehcolor.util.ImageUtil;
 ...
 */
public class AiWorksBokehColorMode extends CameraModeBase implements CaptureDataCallback,
        DeviceCallback, PreviewSizeCallback, IMemoryManager.IMemoryListener,
        CaptureImageSavedCallback {
    private static final LogUtil.Tag TAG = new LogUtil.Tag(AiWorksBokehColorMode.class.getSimpleName());
    ...
    protected IAiWorksBokehColorDeviceController mIAiWorksBokehDeviceController;
    protected AiWorksBokehColorModeHelper mAiWorksBokehModeHelper;
    ...
    //private ConditionVariable mShutterDone = new ConditionVariable(true);
    ...
    public void init(@Nonnull IApp app, @Nonnull ICameraContext cameraContext,
        ...
        AiWorksBokehColorDeviceControllerFactory deviceControllerFactory = new AiWorksBokehColorDeviceControllerFactory();
        ...
        mAiWorksBokehModeHelper = new AiWorksBokehColorModeHelper(cameraContext);
        LogHelper.d(TAG, "[init]- ");
    }
    ...
    public void resume(@Nonnull DeviceUsage deviceUsage) {
        ...
        //bv wuyonglin add for aiworks facebeauty and bokeh 20200722 start
        //modelPath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/Android/data/" + mIApp.getActivity().getPackageName() + "/AIWorksModels";
        ...
        LogHelper.d(TAG, "new onBokehParameterChanged start setFNum resume fNum ="+fNum);
        ...
        LogHelper.d(TAG, "new onBokehParameterChanged setFNum resume fNum ="+fNum);
    ...
    public void onDataReceived(DataCallbackInfo dataCallbackInfo) {
        ...
        /*if(CameraUtil.isWaterMarkOn(mDataStore,mActivity)){
            ...
            WaterMarkUtil.yuvAddWaterMark(mActivity, yuv, picutreW, picutreH, jpegRotation);
            LogHelper.i(TAG,"isWaterMarkOn on - ");
        }else{
    ...
    public void onCameraOpened(String cameraId) {
        ...
        if(Config.isFrontBokehSupport(mIApp.getActivity())){
            relation = AiWorksBokehColorRestriction.getFrontRestriction().getRelation("on", true);
        }else{
            relation = AiWorksBokehColorRestriction.getRestriction().getRelation("on", true);
        }
    ...
    public void onPreviewSizeReady(Size previewSize) {
        LogHelper.i(TAG, "getPreviewSize4 [onPreviewSizeReady] previewSize: " + previewSize.toString());
    ...
    private void prepareAndOpenCamera(boolean needOpenCameraSync, String cameraId,
        ...
        //prepare device info.
        AiWorksBokehColorDeviceInfo info = new AiWorksBokehColorDeviceInfo();
    ...
    private void doCameraSelect(String oldCamera, String newCamera) {
        ...
        LogHelper.d(TAG, "[doCameraSelect]- setFNum fNum ="+fNum);
        mPortraitEffect.setFNum(fNum);
        mPortraitEffect.setOrientation(mIApp.getGSensorOrientation());
        //bv wuyonglin add for aiworks facebeauty and bokeh 20200722 end
    ...
    private class MyStatusChangeListener implements StatusChangeListener {
        ...
        public void onStatusChanged(String key, String value) {
            LogHelper.i(TAG, "getPreviewSize4 [onStatusChanged] key = " + key + ",value = " + value);
            /*-- modify by bv liangchangwei for fixbug 3680--*/
    ...
    private class MyPhotoEffectListener implements PhotoEffectListener {
        ...
        public void onPortraitApply(byte[] yuv, int format, int width, int height) {
            ...
            if (yuv != null) {
                byte[] data;
                if (format == android.graphics.ImageFormat.FLEX_RGBA_8888) {
                    data = YuvEncodeJni.getInstance().Rgb2Jpeg(yuv, width, height, 100, 1);
            LogHelper.i(TAG, "onPortraitApply1 data = " + data);
                } else {
                    data = YuvEncodeJni.getInstance().EncodeYuvToJpeg(yuv, format,
                            width, height, 100, mJpegRotation);
            LogHelper.i(TAG, "onPortraitApply2 updateThumbnail jpegRotation data = " + data);
                }
                saveData(data);
            }
        ...
        public void onPortraitApply(Bitmap result) {
            LogHelper.i(TAG, "onPortraitApply Bitmap");
            //mShutterDone.open();
            if (result != null) {
