...
public class FocusViewController {
     ...
     */
    public void showActiveFocusAt(final int viewX, final int viewY, boolean locked) {
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (mFocusView == null) {
                ...
                mFocusView.startActiveFocus(locked);
            }
        });
    }

    //adb by huangfei for front touch AE start
    public void showFrontTouchAe(final int viewX, final int viewY) {
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (mFocusView == null) {
