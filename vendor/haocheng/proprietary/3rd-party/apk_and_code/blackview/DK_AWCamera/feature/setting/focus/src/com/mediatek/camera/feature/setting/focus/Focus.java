...
public class Focus extends SettingBase implements
    ...
    //*/ hct.huangfei, 20210401.focus sound.
    ...
    private int mQuickSwitcherHeight;
    // bv wuyonglin add for focus view show at not right position 20191224 start
    private int mQuickSwitcherHeight1;
    private boolean is1to1;
    // bv wuyonglin add for focus view show at not right position 20191224 end
    ...
    //start, wangsenhao, fix FocusView show area bug, 2020.04.28
    private int mDefaultDisplayHigh;
    private int mBottomMargin;
    //end, wangsenhao, fix FocusView show area bug, 2020.04.28
    //adb by huangfei for front touch AE start
    private boolean mFrontTouchAeSupport;
    //adb by huangfei for front touch AE end
    ...
    public void init(IApp app,
        ...
        mLockIndicatorHint.mType = IAppUi.HintType.TYPE_AUTO_HIDE;
        mLockIndicatorHint.mDelayTime = 3000;
        ...
        //start, wangsenhao, fix FocusView show area bug, 2020.04.28
        mDefaultDisplayHigh = mApp.getActivity().getWindowManager().getDefaultDisplay().getHeight();
        mBottomMargin = mApp.getActivity().getResources().getDimensionPixelOffset(R.dimen.camera_shutter_bar_height);
        //end, wangsenhao, fix FocusView show area bug, 2020.04.28

        //adb by huangfei for front touch AE start
        mFrontTouchAeSupport = mApp.getActivity().getResources().getBoolean(R.bool.config_front_touch_ae_support)
		|| CameraUtil.getDeviceModel().equals("BL6000Pro");
        //adb by huangfei for front touch AE end

        density = mApp.getActivity().getResources().getDisplayMetrics().density;
        //bv wuyonglin add for screen 2300px adjust all icon position 20201024 start
        ...
        mQuickSwitcherHeight = mApp.getActivity().getResources().getDimensionPixelSize(R.dimen.quick_switcher_height);
        if (mScreenHeight == 2400) {
        mQuickSwitcherHeight = mApp.getActivity().getResources().getDimensionPixelSize(R.dimen.quick_switcher_height_2400px);
        }
        LogHelper.d(mTag, "[init] mQuickSwitcherHeight " + mQuickSwitcherHeight+" density ="+density+" mDefaultDisplayHigh ="+mDefaultDisplayHigh);
        //bv wuyonglin add for screen 2300px adjust all icon position 20201024 end
    }
    ...
    public boolean onSingleTapUp(float x, float y) {
        //add by huangfei for mf start
        if(mProMode.equals(mCurrentMode)&&Config.isMFSupport(mActivity)){
            return false;
        }
        //add by huangfei for mf end

        //adb by huangfei for front touch AE start
        if("1".equals(mAppUi.getCameraId()) && mFrontTouchAeSupport){
            mFocusViewController.showFrontTouchAe((int) x, (int) y -mQuickSwitcherHeight1);
        }
        //adb by huangfei for front touch AE end
	...
	if (!is1to1) {	// bv wuyonglin add for focus view show at not right position 20191224
        if (y <= mQuickSwitcherHeight1 || y >= mQuickSwitcherHeight1 + 480*density) {
            LogHelper.w(mTag, "[onSingleTapUp] ignore,over area");
            return false;
        }
	// bv wuyonglin add for focus view show at not right position 20191224 start
	} else {
        if (y <= mQuickSwitcherHeight1 || y >= mQuickSwitcherHeight1 + 360*density) {
            LogHelper.w(mTag, "[onSingleTapUp] 2 ignore,over area");
            return false;
        }
	}
	// bv wuyonglin add for focus view show at not right position 20191224 end
        ...
        if (mNeedShowFocusUi) {
            ...
            if(isBokehMode()){
               ICameraMode mICameraMode = mApp.getCurrentCameraMode();
                       mICameraMode.showApertureView((int) x, (int) y -mQuickSwitcherHeight1);	// bv wuyonglin modify for aperture view show at not right position 20200227				
            }else{
                //mFocusViewController.showActiveFocusAt((int) x, (int) y);
                mFocusViewController.showActiveFocusAt((int) x, (int) y -mQuickSwitcherHeight1, false);	// bv wuyonglin modify for focus view show at not right position 20191224
            }
            //*/
        }
	...
	// bv wuyonglin add for over area long Press should not show AE/AF locked 20200310 start
	if (!is1to1) {
            if (y <= mQuickSwitcherHeight || y >= mQuickSwitcherHeight + 480*density) {
                LogHelper.w(mTag, "[onLongPress] ignore,over area");
                return false;
            }
	} else {
            if (y <= mQuickSwitcherHeight1 || y >= mQuickSwitcherHeight1 + 360*density) {
                LogHelper.w(mTag, "[onLongPress] 2 ignore,over area");
                return false;
            }
	}
	// bv wuyonglin add for over area long Press should not show AE/AF locked 20200310 end
            ...
            case ACTIVE_UNFOCUSED:
                ...
                if (mNeedTriggerShutterButton) {
                    ...
                    if (mNeedPlayFocusSound) {
                        //*/ hct.huangfei, 20210401.focus sound.
                        //mCameraContext.getSoundPlayback().play(ISoundPlayback.FOCUS_COMPLETE);
                        LogHelper.d(mTag, "[onFocusStateUpdate] mTouchFocus: " + mTouchFocus);
                        if(Config.isFocusSoundSupport(mActivity) && mTouchFocus){
                            mCameraContext.getSoundPlayback().play(ISoundPlayback.FOCUS_COMPLETE);
                        }
                        //*/
                    }
	...
	// bv wuyonglin add for focus view show at not right position 20191224 start
	if (previewRect.bottom == 360 * density) {
            // bv wuyonglin add for focus view show at not right position 20200615 start
            //mQuickSwitcherHeight1 = mQuickSwitcherHeight + (int)(120 * density);
            if (mScreenHeight == 2300) {
            mQuickSwitcherHeight1 = mApp.getActivity().getResources().getDimensionPixelSize(R.dimen.top_margin_1_1_2300px);
            } else if (mScreenHeight == 2400) {
            mQuickSwitcherHeight1 = mApp.getActivity().getResources().getDimensionPixelSize(R.dimen.top_margin_1_1_2400px);
            } else {
            mQuickSwitcherHeight1 = mApp.getActivity().getResources().getDimensionPixelSize(R.dimen.top_margin_1_1);
            }
            // bv wuyonglin add for focus view show at not right position 20200615 end
            is1to1 = true;
            LogHelper.d(mTag, "[setPreviewRect] 1 mQuickSwitcherHeight1 ="+mQuickSwitcherHeight1);
	} else if (previewRect.bottom == (previewRect.right - previewRect.left) * 2) {
            mQuickSwitcherHeight1 = CameraUtil.getRealStatusBarHeight(mApp.getActivity());
            if (mScreenHeight == 2400) {
                mQuickSwitcherHeight1 = mApp.getActivity().getResources().getDimensionPixelSize(R.dimen.top_margin_full_2400px);
            }
            LogHelper.d(mTag, "[setPreviewRect] 2 mQuickSwitcherHeight1 ="+mQuickSwitcherHeight1);
	    is1to1 = false;
	} else {
            mQuickSwitcherHeight1 = mQuickSwitcherHeight + CameraUtil.getRealStatusBarHeight(mApp.getActivity());
            LogHelper.d(mTag, "[setPreviewRect] 3 mQuickSwitcherHeight1 ="+mQuickSwitcherHeight1+" mQuickSwitcherHeight ="+mQuickSwitcherHeight);
	    is1to1 = false;
	}
	// bv wuyonglin add for focus view show at not right position 20191224 end
    ...
    private boolean checkAfEnv() {
        ...
        if(mAppUi.isHdrPictureProcessing()){
            LogHelper.w(mTag, "[onSingleTapUp] ignore,HdrPicture is Processing");
            return false;
        }
        return true;
    }
    ...
    private void triggerAfLock() {
        ...
        if (mNeedShowFocusUi) {
            mFocusViewController.showActiveFocusAt(mLockPoint.x, mLockPoint.y - mQuickSwitcherHeight1, true);    // bv wuyonglin modify for focus view show at not right position 20191224
            mAppUi.hideScreenHint(mLockIndicatorHint);
    ...
    public boolean isBokehMode(){
        if("HctBokeh".equals(mAppUi.getCurrentMode())){
            return true;
        }else{
            return false;
        }
