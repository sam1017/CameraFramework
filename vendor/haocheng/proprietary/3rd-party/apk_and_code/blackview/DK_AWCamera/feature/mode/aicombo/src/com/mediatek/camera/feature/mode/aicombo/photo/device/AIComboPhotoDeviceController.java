...
public class AIComboPhotoDeviceController extends Device2Controller implements
        IAIComboPhotoDeviceController,
        CaptureSurface.ImageCallback,
        ISettingManager.SettingDevice2Requester {
    private static final Tag TAG = new Tag(AIComboPhotoDeviceController.class.getSimpleName());
     ...
     */
    public AIComboPhotoDeviceController(@Nonnull Activity activity, @Nonnull ICameraContext context) {
        //LogHelper.d(TAG, "[AIComboPhotoDeviceController] +");
        mActivity = activity;
    ...
    private void initSettings() throws CameraAccessException {
        sRelation = AIComboPhotoRestriction.getRestriction().getRelation("on", true);
        if (sRelation != null) {
    ...
    private void updateCameraState(CameraState state) {
        //LogHelper.d(TAG, "[updateCameraState] new state = " + state + " old =" + mCameraState,new Throwable());
        mLockState.lock();
        try {
            mCameraState = state;
        } finally {
    ...
    private void configureSession(boolean isFromOpen) {
        //LogHelper.i(TAG, "[configureSession] +" + ", isFromOpen :" + isFromOpen,new Throwable());
        mDeviceLock.lock();
        mFirstFrameArrived = false;
        try {
    ...
    private Size getTargetPreviewSize(double ratio) {
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
                if (previewSizes[i].getWidth() <= PREVIEW_SIZE_KEY_VALUE[1] &&
                    previewSizes[i].getHeight() <= PREVIEW_SIZE_KEY_VALUE[0]) {
                       sizes.add(j, new Size(previewSizes[i].getWidth(), previewSizes[i].getHeight()));
                       j++;
                }
            }
