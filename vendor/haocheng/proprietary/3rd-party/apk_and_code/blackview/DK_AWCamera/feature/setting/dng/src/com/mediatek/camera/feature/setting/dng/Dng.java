...
public class Dng extends SettingBase implements IAppUiListener.OnShutterButtonListener
    ...
    public boolean onShutterButtonClick() {
        if (mIsDngLowMemory||mIsDngCreatorBusy) {
            LogHelper.d(TAG, "[onShutterButtonClick] dng creator " +
                    " busy = "+mIsDngCreatorBusy +
                    " | low memory = "+ mIsDngLowMemory);
            return true;
        }
    ...
    public void overrideValues(@Nonnull String headerKey, String currentValue,
        ...
        if (CameraModeBase.MODE_DEVICE_STATE_PREVIEWING.equals(mCurrentModeDeviceState)){
            mDngDeviceCtrl.notifyOverrideValue(curValue,mCurrentModeKey);
        }
        mDngViewCtrl.setEntryValue(getEntryValues());
    ...
    private MediaSaver.MediaSaverListener mMediaSaverListener
        ...
        public void onFileSaved(Uri uri) {
            LogHelper.d(TAG, "[onFileSaved] uri = " + uri +
                                    "mCapturingNumber = " + mCapturingNumber);
            synchronized (mCaptureNumberSync) {
                mCapturingNumber --;
                if (mCapturingNumber == 0) {
                    mIsDngLowMemory = false;
                }
            }
        }
    };
    ...
    private IDngConfig.OnDngValueUpdateListener mDngValueListener
        ...
        public void onSaveDngImage(byte[] data, Size rawSize) {
            ...
            synchronized (mCaptureNumberSync) {
                mCapturingNumber ++;
                mMemoryManager.checkOneShotMemoryAction(data.length);
            }
            LogHelper.d(TAG, "[onSaveDngImage] mCapturingNumber = " + mCapturingNumber);
        }
