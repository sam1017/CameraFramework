...
import com.mediatek.camera.feature.mode.hctbokeh.device.IHctBokehDeviceController;
import com.mediatek.camera.feature.mode.hctbokeh.device.IHctBokehDeviceController.DataCallbackInfo;
import com.mediatek.camera.feature.mode.hctbokeh.device.IHctBokehDeviceController.DeviceCallback;
import com.mediatek.camera.feature.mode.hctbokeh.device.IHctBokehDeviceController.CaptureDataCallback;
import com.mediatek.camera.feature.mode.hctbokeh.device.IHctBokehDeviceController.PreviewSizeCallback;
import com.mediatek.camera.feature.mode.hctbokeh.device.HctBokehDeviceControllerFactory;
import com.mediatek.camera.feature.mode.hctbokeh.device.IHctBokehDeviceController.CaptureImageSavedCallback;
...
import com.mediatek.camera.feature.mode.hctbokeh.view.BokehViewCtrl;
...
import com.mediatek.camera.common.relation.Relation;
import com.mediatek.camera.feature.mode.hctbokeh.HctBokehRestriction;
 ...
 */
public class HctBokehMode extends CameraModeBase implements CaptureDataCallback,
        DeviceCallback, PreviewSizeCallback, IMemoryManager.IMemoryListener,
        CaptureImageSavedCallback {
    private static final LogUtil.Tag TAG = new LogUtil.Tag(HctBokehMode.class.getSimpleName());
    ...
    protected IHctBokehDeviceController mIHctBokehDeviceController;
    protected HctBokehModeHelper mHctBokehModeHelper;
    ...
    private BokehViewCtrl mBokehViewCtrl;
    ...
    public void init(@Nonnull IApp app, @Nonnull ICameraContext cameraContext,
		...
		mBokehViewCtrl = new BokehViewCtrl(cameraContext);
        ...
        HctBokehDeviceControllerFactory deviceControllerFactory = new HctBokehDeviceControllerFactory();
        mIHctBokehDeviceController = deviceControllerFactory.createDeviceController(app.getActivity(),
        ...
        mHctBokehModeHelper = new HctBokehModeHelper(cameraContext);
        LogHelper.d(TAG, "[init]- ");
    ...
    public void resume(@Nonnull DeviceUsage deviceUsage) {
        ...
        mIHctBokehDeviceController.queryCameraDeviceManager();
    ...
    public void pause(@Nullable DeviceUsage nextModeDeviceUsage) {
        ...
        if (mNeedCloseCameraIds.size() > 0) {
            ...
            mIHctBokehDeviceController.closeSession();
        } else{
            clearAllCallbacks(mCameraId);
            mIHctBokehDeviceController.stopPreview();
        }
        LogHelper.i(TAG, "[pause]-");
    }
    ...
    public void unInit() {
        ...
        mBokehViewCtrl = null;
        mIHctBokehDeviceController.destroyDeviceController();
    }
    ...
    protected boolean doShutterButtonClick() {
        ...
        boolean isDeviceReady = mIHctBokehDeviceController.isReadyForCapture();
        ...
        if (storageReady && isDeviceReady && mIsResumed
            ...
            mIHctBokehDeviceController.updateGSensorOrientation(mIApp.getGSensorOrientation());
            mIHctBokehDeviceController.takePicture(this);
        ...
        if (mIsResumed) {
            ...
            if (mCameraApi == CameraApi.API1) {
                if (needRestartPreview && !mIsMatrixDisplayShow) {
                    mIHctBokehDeviceController.startPreview();
                }
            }
        }
        //update thumbnail
...
/*                HeifHelper heifHelper = new HeifHelper(mICameraContext);
    ...
    public void onCameraOpened(String cameraId) {
        ...
        //Relation relation = HctBokehRestriction.getRestriction().getRelation("on", true);
        Relation relation = null;
        if(Config.isFrontBokehSupport(mIApp.getActivity())){
            relation = HctBokehRestriction.getFrontRestriction().getRelation("on", true);
        }else{
            relation = HctBokehRestriction.getRestriction().getRelation("on", true);
        }
    ...
    private void prepareAndOpenCamera(boolean needOpenCameraSync, String cameraId,
        ...
        mIHctBokehDeviceController.setDeviceCallback(this);
        mIHctBokehDeviceController.setPreviewSizeReadyCallback(this);
        mIHctBokehDeviceController.setSavedDataCallback(this);
        //prepare device info.
        HctBokehDeviceInfo info = new HctBokehDeviceInfo();
    ...
    private void clearAllCallbacks(String cameraId) {
        mIHctBokehDeviceController.setPreviewSizeReadyCallback(null);
    ...
    private void updatePictureSizeAndPreviewSize(Size previewSize) {
        ...
        if (size != null && mIsResumed) {
            ...
            mIHctBokehDeviceController.setPictureSize(new Size(mCaptureWidth, mCaptureHeight));
    ...
    private void saveData(byte[] data) {
        if (data != null) {
            ...
            ContentValues contentValues = mHctBokehModeHelper.createContentValues(data,
    ...
    private class SurfaceChangeListener implements ISurfaceStatusListener {
        ...
        public void surfaceAvailable(Object surfaceObject, int width, int height) {
            LogHelper.d(TAG, "surfaceAvailable,device controller = " + mIHctBokehDeviceController
                    + ",w = " + width + ",h = " + height);
            if (mModeHandler != null) {
                mModeHandler.post(new Runnable() {
                    @Override
                    public void run() {
            if (mIHctBokehDeviceController != null && mIsResumed) {
                mIHctBokehDeviceController.updatePreviewSurface(surfaceObject);
            }
        }
                });
            }
        ...
        public void surfaceChanged(Object surfaceObject, int width, int height) {
            LogHelper.d(TAG, "surfaceChanged, device controller = " + mIHctBokehDeviceController
                    + ",w = " + width + ",h = " + height);
            if (mModeHandler != null) {
                mModeHandler.post(new Runnable() {
                    @Override
        ...
        public void surfaceDestroyed(Object surfaceObject, int width, int height) {
            LogHelper.d(TAG, "surfaceDestroyed,device controller = " + mIHctBokehDeviceController);
        }
    }
    ...
    private class MyStatusChangeListener implements StatusChangeListener {
        ...
        public void onStatusChanged(String key, String value) {
            ...
            if (KEY_PICTURE_SIZE.equalsIgnoreCase(key)) {
                ...
                mIHctBokehDeviceController.setPictureSize(new Size(mCaptureWidth, mCaptureHeight));
                Size previewSize = mIHctBokehDeviceController.getPreviewSize((double) mCaptureWidth /
                        mCaptureHeight);
                int width = previewSize.getWidth();
                int height = previewSize.getHeight();
                if (width != mPreviewWidth || height != mPreviewHeight) {
                ...
                mIHctBokehDeviceController.setFormat(value);
                LogHelper.i(TAG, "[onStatusChanged] key = " + key + ", set sCaptureFormat = " + value);
            }
        }
    }
    private BokehViewCtrl.ViewChangeListener mViewChangeListener
            = new BokehViewCtrl.ViewChangeListener() {
        @Override
        public void onBokehParameterChanged(int level,int x,int y,int radius) {
            mIHctBokehDeviceController.setBokehParameter(level,x,y,radius);
        }
