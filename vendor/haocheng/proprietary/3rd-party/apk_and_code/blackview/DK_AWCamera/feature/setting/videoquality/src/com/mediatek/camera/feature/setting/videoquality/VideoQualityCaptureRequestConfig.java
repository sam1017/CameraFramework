    ...
    private void generateSupportedList(int cameraId,
        ...
        for (int i = 0; i < defMatrix.length && supported.size() < QUALITY_MAX_NUMBER; i++) {
            if (CamcorderProfile.hasProfile(cameraId, defMatrix[i])
                ...
                if (!"SlowMotion".equals(mVideoQuality.getCurrentMode())) {
                    supported.add(Integer.toString(defMatrix[i]));
                    //bv wuyonglin add for 60fps 20201124 start
                    if (defMatrix[i] == CamcorderProfile.QUALITY_1080P && isFPS60Support(Integer.toString(defMatrix[i]))) {
                        supported.add("60");
                    }
                    //bv wuyonglin add for 60fps 20201124 end
                } else {
                    if (defMatrix[i] == CamcorderProfile.QUALITY_1080P) {
                                supported.add(Integer.toString(defMatrix[i]));
                    }
                }
