...
//add by huangfei for gestural navigationbar start
import static android.view.WindowManagerPolicyConstants.NAV_BAR_MODE_GESTURAL;
//add by huangfei for gestural navigationbar end
...
public final class CameraUtil {
    ...
    public static boolean MTKCAM_AIWORKS_HDR_SUPPORT = true; //add by liangchangwei for Aiworks HDR
    public static boolean MTKCAM_AIWORKS_VIDEO_HDR_SUPPORT = true; //add by liangchangwei for Aiworks Video HDR
    /* add by liangchangwei for HDR Video start--*/
    public static boolean isVideo_HDR_on = false;
    public static boolean isVideo_HDR_changing = false;
    /* add by liangchangwei for HDR Video end--*/
    /*--add by bv liangchangwei for fix bug 3752--*/
    public static boolean isVideo_opening = false;
    /*--add by bv liangchangwei for fix bug 3752--*/

    public static boolean isRTL = false;

    public static boolean is_videoHdr_Force = false;
    //*/ hct.huangfei, 20201027. continuous shot numbers.
    public static final String CAMERA_SHARED_PREFS = "com.mediatek.camera_preferences";
    public static final String CAMERA_SHARED_PREFS_BACK = "com.mediatek.camera_preferences_0";
    public static final String CAMERA_SHARED_PREFS_FRONT = "com.mediatek.camera_preferences_1";
    //*    
    ...
    public static final String KEY_VOLUME_KEY = "key_volume_key";
    //*/
    ...
    **/
    ...
    public static boolean isEdgeToEdgeEnabled(Context context) {
        ...
        try {
            newContext = context.createPackageContext(context.getPackageName(),0);
        ...
        int resId = res.getIdentifier("config_navBarInteractionMode", "integer", "android");
        if (resId != 0) {
            return res.getInteger(resId) == NAV_BAR_MODE_GESTURAL;
        }
        return false;
        //*/
    }
    ...
    public static int getNavigationBarHeight(Activity activity) {
        if (isHasNavigationBar(activity)) {
            ...
            if(isEdgeToEdgeEnabled(activity)){
                navigationBarHeight = activity.getResources().getDimensionPixelOffset(R.dimen.navigationbar_hide_bottom_margin);
                return navigationBarHeight;
            }
            //get navigation bar height.
            int resourceId = activity.getResources().getIdentifier(
                    "navigation_bar_height", "dimen", "android");
            navigationBarHeight = activity.getResources()
        ...
        for (int mode : availableAfModes) {
            switch (mode) {
                case CameraMetadata.CONTROL_AF_MODE_AUTO:
                    //add by huangfei for remove manual foucs of front camera start
                    break;
                    //add by huangfei for remove manual foucs of front camera end
    ...
    public static int isNotchScreenSupport(){
        int support = SystemProperties.getInt("ro.hct_support_notch_screen", 0);
        LogHelper.e(TAG, "isNotchScreenSupport = " + support);
        return support;
    }
    public static int getStatusBarHeight(Context mContext) {
        int height = 0;
        if(isNotchScreenSupport()!=1){
            return height;
        }
    ...
    public static boolean isCameraSoundOn(Context context){
        //start, wangsenhao, default camera sound value, 2019.06.03
        int resId = context.getResources().getIdentifier("config_default_camerasound_on", "bool", context.getPackageName());
        boolean isSoundon = context.getResources().getBoolean(resId);
        return context.getSharedPreferences("camera_sound",
                Context.MODE_PRIVATE).getBoolean("camera_sound_value", isSoundon);
    }
    //add by huangfei for camerasound end

    /* hct.wangsenhao, underwater camera @{ */
    public static int getUnderWaterSupport(Context mContext){
        int underWaterEnable = Settings.Global.getInt(mContext.getContentResolver(), "under_water_value", 0);
        return underWaterEnable;
    }
    /* }@ hct.wangsenhao */
    //*/ hct.huangfei, 20201106. continuousshot unsupprit tips of watermark.
    public static boolean isWaterMarkOn(DataStore dataStore,Activity activity){
        if(dataStore==null){
            LogHelper.e(TAG, "isWaterMarkOn dataStore is null");
            return false;
        }
        boolean waterOn = dataStore.getValue(KEY_WATERMARK, "off", dataStore.getGlobalScope()).equals("on");
        boolean waterEnable = dataStore.getValue(KEY_WATERMARK_ENABLE, "0", dataStore.getGlobalScope()).equals("1");
        if(Config.hctHalWaterMarkSupport()&&waterOn&&waterEnable){
            return true;
        }
        return false;
    }
    ...
    //addb by huangfei for long screen layout start
    public static boolean isLongScreen(Activity activity){
        WindowManager wm = (WindowManager) activity.getSystemService(Context.WINDOW_SERVICE);
        Display display = wm.getDefaultDisplay();
        Point point = new Point();
        display.getRealSize(point);
        int panelHeight = Math.max(point.x, point.y);
        int panelWidth = Math.min(point.x, point.y);
        double ratio = (double) panelHeight/panelWidth;
        if(ratio>2.05){
            return true;
        }
        return false;
    }
    //addb by huangfei for long screen layout end
    //*/ hct.huangfei, 20201030. add customize zoom
    public static boolean isZoomViewCustomizeSupport(Activity activity){
        int resId = activity.getResources().getIdentifier("config_zoom_view_customize_support",
                "bool", activity.getPackageName());
        return activity.getResources().getBoolean(resId);
    }
    //*/

    //*/ hct.huangfei, 20201210.add volume key function.
    public static String getVolumeKeyValue(DataStore dataStore,Activity activity){
        int resId = activity.getResources().getIdentifier("volume_key_default_value",
        "string", activity.getPackageName());
        String defaultValue = activity.getResources().getString(resId);
        if(dataStore==null){
            LogHelper.e(TAG, "getVolumeKeyValue dataStore is null");
            return defaultValue;
        }
        String value = dataStore.getValue(KEY_VOLUME_KEY, defaultValue, dataStore.getGlobalScope());
        return value;
    }
    //*/

    //*/ hct.huangfei, 20210105.numbers of picture size.
    public static boolean is18_9PreviewSizeSupport(Context context) {
   ...
   //bv wuyonglin add for adjust all icon position 20200612 start
    public static int getRealStatusBarHeight(Context mContext) {
        int height = 0;
        int resId = mContext.getResources().getIdentifier("status_bar_height", "dimen", "android");
