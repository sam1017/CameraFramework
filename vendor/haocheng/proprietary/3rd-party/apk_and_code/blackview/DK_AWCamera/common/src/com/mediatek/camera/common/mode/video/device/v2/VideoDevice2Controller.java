...
import com.mediatek.camera.feature.setting.hdr.Hdr;
...
public class VideoDevice2Controller extends Device2Controller
    ...
    private VideoMode.HDRChangeListener mHDRChangeListener;
    ...
    //*/ hct.huangfei, 20210113.video mode open fail.
    ...
    public VideoDevice2Controller(@Nonnull Activity activity, @Nonnull ICameraContext context) {
        ...
        //add by liangchangwei for Video HDR start
        String DUMP_PATH = Environment.getExternalStorageDirectory().getPath() + "/livehdr/";
        String KEY_CONFIG = "pref_live_hdr_config_key";
        String PREFERENCES_FILE_NAME = "customized_video_camera_preferences.xml";
        String model = get("ro.product.model");
        LogHelper.i(TAG,"model = " + model);
        mDataStore = context.getDataStore();
        mHdrStatus = mDataStore.getValue("key_hdr_video", "off", mDataStore.getGlobalScope());
        String filepath = model + "/" + PREFERENCES_FILE_NAME;
        ...
        HashMap<String, String> customPreferences = CameraPreferences.loadCustomizedCameraPreferences(
                mActivity, DUMP_PATH, filepath);
        if (customPreferences != null) {
            String configJsonStr = customPreferences.get(KEY_CONFIG);
            LogHelper.i(TAG,"VideoDevice2Controller configJsonStr = " + configJsonStr);
            LiveHDRConfig.configData(configJsonStr);
        }
    ...
    public void createAndChangeRepeatingRequest() {
        LogHelper.i(TAG,"createAndChangeRepeatingRequest mHdrStatus = " + mHdrStatus);
        String HdrStatus = mDataStore.getValue("key_hdr_video", "off", mDataStore.getGlobalScope());
        LogHelper.i(TAG,"createAndChangeRepeatingRequest HdrStatus = " + HdrStatus);
        if(!mHdrStatus.equals(HdrStatus)){
            LogHelper.i(TAG,"createAndChangeRepeatingRequest HdrStatus change to " + HdrStatus);
            mHdrStatus = HdrStatus;
/*            if(mHDRChangeListener != null){
                if(mHdrStatus.equals("on")){
                    mHDRChangeListener.onHDRChanged(true);
                }else{
                    mHDRChangeListener.onHDRChanged(false);
                }
            }*/
        }
    ...
    public void requestRestartSession() {
        ...
        try {
            LogHelper.i(TAG, "[requestRestartSession] + cameraId : ");
            abortOldSession();
            updatePictureSize();
    ...
    @Override
    public void setHDRChangeListener(VideoMode.HDRChangeListener listener){
        mHDRChangeListener = listener;
    }
    ...
    public void updatePreviewSurface(Object surfaceObject) {
        if (surfaceObject != null) {
            ...
            mVideoHandler.sendEmptyMessageDelayed(HANDLER_UPDATE_PREVIEW_SURFACE, 200);//modify by huangfei for disconnect camera when video quality change start
        } else {
    ...
    @Override
    public void release() {
        mVideoHandler.removeMessages(HANDLER_UPDATE_PREVIEW_SURFACE);//modify by huangfei for disconnect camera when video quality change start
    ...
    private void configureSession() {
        ...
        LogHelper.i(TAG,"capture = " + capture);
        if (capture != null) {
        ...
        try {
            ...
            LogHelper.i(TAG,"surfaces.size = " + surfaces.size());
    ...
    private void initProfile() {
        ...
        //bv wuyonglin add for bug5481 20210422 start
        if(quality < 4){
            quality = 4;
        }
        //bv wuyonglin add for bug5481 20210422 end
    ...
    private void reviseVideoCapability() {
        ...
        String fps60 = mDataStore.getValue(KEY_FPS60, "off", "_preferences_0");
        //bv wuyonglin add for 60fps 20201124 end
