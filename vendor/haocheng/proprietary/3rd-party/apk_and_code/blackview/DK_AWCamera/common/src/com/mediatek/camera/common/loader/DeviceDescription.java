...
public class DeviceDescription {
    ...
    private static final String TPI_FB_KEY
            = "com.mediatek.streamingfeature.demofb";
    private static final String TPI_EIS_KEY
            = "com.mediatek.streamingfeature.demoeis";
    private static final String TPI_ASYNC_KEY
            = "com.mediatek.streamingfeature.demoasd";
    ...
    private CaptureRequest.Key<int[]> mKeyTPIFB;
    private CaptureRequest.Key<int[]> mKeyTPIEis;
    private CaptureRequest.Key<int[]> mKeyTPIAsync;
    ...
    public void storeCameraCharacKeys(CameraCharacteristics cs) {
        ...
        for (CaptureRequest.Key<?> requestKey : sessionKeyList) {
            ...
            if (TPI_FB_KEY.equals(requestKey.getName())){
                mKeyTPIFB = (CaptureRequest.Key<int[]>)requestKey;
            }
            if (TPI_EIS_KEY.equals(requestKey.getName())){
                mKeyTPIEis = (CaptureRequest.Key<int[]>)requestKey;
            }
            if (TPI_ASYNC_KEY.equals(requestKey.getName())){
                mKeyTPIAsync = (CaptureRequest.Key<int[]>)requestKey;
            }
        }
        ...
        for (CaptureRequest.Key<?> requestKey : requestKeyList) {
            ...
            //*/ hct.huangfei, 20201027. add camera mirror.
            } else if (requestKey.getName().equals(FLIP_KEY_MODE_REQUEST)){
    ...
    public CaptureRequest.Key<int[]> getKeyTpiFb() {
        return mKeyTPIFB;
    }

    public CaptureRequest.Key<int[]> getKeyTpiEis() {
        return mKeyTPIEis;
    }

    public CaptureRequest.Key<int[]> getKeyTpiAsync() {
        return mKeyTPIAsync;
    }
    
    //*/ hct.huangfei, 20201027. add camera mirror.
    public CaptureRequest.Key<int[]> getKeyFlipRequestMode() {
        return mKeyFlipMode;
    }
    //*/
}
