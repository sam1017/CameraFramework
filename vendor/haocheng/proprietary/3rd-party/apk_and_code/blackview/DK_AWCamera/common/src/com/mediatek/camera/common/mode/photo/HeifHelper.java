...
public class HeifHelper {
    ...
    public static final String CAPTURE_TYPE_HEIF = "heif";
    ...
    public static byte[] getHeicBuffer(Image image) {
        ...
        LogHelper.d(TAG, "<getHeicBuffer> image = " + image
    ...
    public static int getCaptureFormat(String formatValue) {
        ...
        if (CAPTURE_TYPE_HEIF.equalsIgnoreCase(formatValue)) {
            if (FormatEntry.HEIF_FLOW == FormatEntry.HEIF_WRITER_FLOW){
    ...
    public ContentValues getContentValues(byte[] data, String fileDirectory, int
            pictureWidth, int pictureHeight) {

        ContentValues values = new ContentValues();
        long dateTaken = System.currentTimeMillis();
        String name = mImageFileName.generateTitle(dateTaken);
        String fileName = mImageFileName.generateTitle(dateTaken) + ".heic";
        int orientation = CameraUtil.getOrientationFromSdkExif(data);

        String mime = "image/heic";
        ...
        LogHelper.d(TAG, "[getContentValues] width : " + pictureWidth + ",height = " +
