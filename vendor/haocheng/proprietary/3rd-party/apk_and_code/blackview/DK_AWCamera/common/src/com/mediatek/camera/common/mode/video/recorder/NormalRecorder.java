...
public class NormalRecorder implements IRecorder {
    ...
    public void init(RecorderSpec spec) {
        ...
        if ((!spec.isHEVC) && (spec.profile.videoCodec == MediaRecorder.VideoEncoder.H264)) {
            MediaCodec mEncoder = null;
            try {
                mEncoder = MediaCodec.createEncoderByType(MediaFormat.MIMETYPE_VIDEO_AVC);
            } catch (IOException e) {
                LogHelper.d(TAG, "createEncoderByType failed");
            }
            if(mEncoder != null) {
                MediaCodecInfo.CodecCapabilities caps = mEncoder.getCodecInfo().getCapabilitiesForType(MediaFormat.MIMETYPE_VIDEO_AVC);
                for (MediaCodecInfo.CodecProfileLevel i : caps.profileLevels) {
                    if (MediaCodecInfo.CodecProfileLevel.AVCLevel41 == i.level) {
                        if (MediaCodecInfo.CodecProfileLevel.AVCProfileHigh == i.profile) {
                            LogHelper.d(TAG, "EncodingProfileLevel  = AVCProfileHigh + AVCLevel41");
