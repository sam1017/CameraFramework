...
import com.mediatek.camera.common.mode.video.glrenderer.SurfaceTextureListener;
import com.mediatek.camera.common.mode.video.glrenderer.SurfaceTextureRenderer;
...
//*/ hct.huangfei, 20201203. add storagepath.
...
public class VideoHelper {
    ...
    private Uri mUri;
    //add by liangchangwei for Video HDR start
    public String model_file_path;
    public static final String MODEL_FILE = "AIWorksModels";

    private SurfaceTextureRenderer surfaceTextureRenderer;
    private LiveHDRDrawer mDrawer;
    //add by liangchangwei for Video HDR end
    /**
     * the constructor.
    ...
    public VideoHelper(ICameraContext context, IApp app, IDeviceController cameraDevice,
        ...
        //add by liangchangwei for Video HDR start
        File dir = mApp.getActivity().getFilesDir();
        model_file_path = dir.getAbsolutePath() + File.separator + MODEL_FILE + File.separator;
        LogHelper.i(TAG,"create HDRNetDrawer");
        mDrawer = new LiveHDRDrawer();
        //add by liangchangwei for Video HDR end
    }
    /**
    ...
    public long getRecorderMaxSize() {
        ...
        long videoSize = mCameraContext.getStorageService().getRecordStorageSpace();
        if (SDCardFileUtils.isSDCardSupport()){
            long limit = (long) (3.99 * 1024 * 1024 * 1024);
            File file = new File(SDCardFileUtils.getSDCardVolumePath());
            long mFreeSpace = file.getFreeSpace();
            if (limit > mFreeSpace){
                videoSize = mFreeSpace;
            }else if (videoSize > limit){
                videoSize = limit;
            }
            LogHelper.d(TAG, "[SDCard]videoSize = " + videoSize);
        }
        return videoSize;		
        //*/
    }
    ...
    public IRecorder.RecorderSpec configRecorderSpec(CamcorderProfile profile, String cameraId,
        ...
        if (VALUE_ON.equals(settingManager.getSettingController().queryValue("key_hdr10"))) {
            recorderSpec.isHEVC = true;
        }else if(VALUE_ON.equals(settingManager.getSettingController().queryValue("key_fps60")) &&
                CamcorderProfile.QUALITY_2160P == (Integer.parseInt(
                        settingManager.getSettingController().queryValue("key_video_quality")))){
            recorderSpec.isHEVC = true;
        }else if(VIDEO_FORMAT_HEVC.equals(settingManager.getSettingController().queryValue("key_video_format"))){
            recorderSpec.isHEVC = true;
        }
        else {
    ...
    public FileDescriptor getVideoFD(){
        ...
        if(SDCardFileUtils.isSDCardSupport()){
            mUri = mActivity.getContentResolver().insert(MediaStore.Video.Media.getContentUri(getSdCardExternalVolumeName()),
                         getContentValuesNewVideo());
        }else{
            mUri = mActivity.getContentResolver().insert(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, getContentValuesNewVideo());
        }
        //*/
    ...
    private class SurfaceChangeListener implements IAppUiListener.ISurfaceStatusListener {
        ...
        public void surfaceAvailable(Object surfaceObject, int width, int height) {
            ...
            String HdrStatus = mDataStore.getValue("key_hdr_video", "off", mDataStore.getGlobalScope());
            LogHelper.i(TAG,"surfaceAvailable HdrStatus = " + HdrStatus);

            if(CameraUtil.MTKCAM_AIWORKS_VIDEO_HDR_SUPPORT && HdrStatus.equals("on")
			 && mApp.getAppUi().getCurrentMode() != null && !mApp.getAppUi().getCurrentMode().equals("SlowMotion")){	//bv wuyonglin modify for bug4063 20210226
                /* add by liangchangwei for HDR Video start--*/
                CameraUtil.isVideo_HDR_on = true;
                /* add by liangchangwei for HDR Video end--*/
                if (surfaceObject != null && surfaceObject instanceof SurfaceTexture) {
                    LogHelper.i(TAG,"init surfaceTextureRenderer");
                    surfaceTextureRenderer = new SurfaceTextureRenderer(((SurfaceTexture)surfaceObject), width, height, mDrawer);
                    surfaceTextureRenderer.acquireSurfaceTexture( height ,width, mSurfaceTextureListener);
                }
            }else{
                /* add by liangchangwei for HDR Video start--*/
                CameraUtil.isVideo_HDR_on = false;
                /* add by liangchangwei for HDR Video end--*/
    ...
    private SurfaceTextureListener mSurfaceTextureListener = new SurfaceTextureListener() {
        ...
        public void onSurfaceTextureAvailable(final SurfaceTexture surface) {
            LogHelper.i(TAG, "onSurfaceTextureAvailable surface = " + surface + " mVideoHandler = " + mVideoHandler);
            ...
            if (mVideoHandler != null) {
                mVideoHandler.post(new Runnable() {
                    ...
                    public void run() {
                        if (mCameraDevice != null) {
                            LogHelper.i(TAG,"updatePreviewSurface surface = " + surface );
                            mCameraDevice.updatePreviewSurface(surface);
                        }
                    }
                });
            }
            LogHelper.i(TAG, "mDrawer.init model_file_path = " + model_file_path);
            mDrawer.init(model_file_path, false, true);
        }

        @Override
        public void firstPreviewReceived() {
        }
    };

    public LiveHDRDrawer getDrawer(){
        return mDrawer;
    }
