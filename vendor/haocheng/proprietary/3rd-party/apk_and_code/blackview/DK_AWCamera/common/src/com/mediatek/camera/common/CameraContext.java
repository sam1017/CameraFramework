...
public class CameraContext implements ICameraContext {
    ...
    public void create(IApp app, Activity activity) {
		...
		mMediaSaver = new MediaSaver(activity, app.getAppUi(), mDataStore);
    ...
    public CamPostAlgo getCamPostAlgo() {
        mCamPostAlgo.init(mActivity);
        return mCamPostAlgo;
    }

    private void startBGCamPostAlgoThread() {
        mBGCamPostAlgoThread = new HandlerThread("BGCamPostAlgo");
        mBGCamPostAlgoThread.start();
        mBGCamPostAlgoHandler = new Handler(mBGCamPostAlgoThread.getLooper());
        if (mBGCamPostAlgoHandler != null) {
           mBGCamPostAlgoHandler.post(startBGCamPostAlgoService);
        }
    }


    private void stopBGCamPostAlgoThread() {
        if (mBGCamPostAlgoThread != null) {
            mBGCamPostAlgoThread.quitSafely();
            mBGCamPostAlgoThread = null;
        }
        if (mBGCamPostAlgoHandler != null) {
            mBGCamPostAlgoHandler = null;
        }
    }

    private Runnable startBGCamPostAlgoService =
        new Runnable() {
           @Override
           public void run() {
               getCamPostAlgo();
