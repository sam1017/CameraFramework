...
//add by huangfei for add pixel start
#define PIXEL_ARRAY_SIZE 22
static int32_t __unused pixel_width_array[PIXEL_ARRAY_SIZE]  = {2832,3456,3264,3840,4032,4000,4608,4928,4160,4864,5184,4608,5376,5664,5312,6080,5696,6544,6944,8000,9216,9792};
static int32_t __unused pixel_height_array[PIXEL_ARRAY_SIZE] =  {2832,3456,2448,2160,2016,3000,2592,2464,3120,2736,2592,3456,3024,2832,3984,3424,4272,3681,3472,6000,5184,4896};
//add by huangfei for add pixel end
...
bool CameraDeviceClient::roundBufferDimensionNearest(int32_t width, int32_t height,
    ...
    for (size_t i = 0; i < streamConfigs.count; i += 4) {
        ...
        //add by huangfei for add pixel start
        bool isAddPixel = false;
        int32_t matchIndex = 0;
        for(int32_t i=0;i<PIXEL_ARRAY_SIZE;i++){
            if(width==pixel_width_array[i]&& height ==pixel_height_array[i]){
                isAddPixel = true;
                matchIndex = i;
                break;
            }
        }
        ALOGE("matchIndex: %d  isAddPixel: %d", matchIndex, isAddPixel);
        //add by huangfei for add pixel end
        ...
        if (fmt == format) {
            ...
            //add by huangfei for add pixel start
                ...
                bestWidth = pixel_width_array[matchIndex];
                bestHeight = pixel_height_array[matchIndex];
                break;                
            //add by huangfei for add pixel end
